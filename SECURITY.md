...
Bailu and also where the boundaries are.

In general Bailu is a coding agent that runs locally within the security boundary
...
Bailu treats the local user account and files writable by that account as inside
the same trust boundary as the Bailu process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Bailu configuration, they can generally influence Bailu or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Bailu grants that write access or crosses
an operating-system privilege boundary.

Bailu relies on users installing trustworthy extensions and loading trustworthy
skills and only to use bailu within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in bailu or another package in
...
- Local code execution or sandboxing behavior (the Bailu coding agent intentionally does not have a sandbox)
- Behavior of bailu extensions or skills installed by the user
...
- Public internet exposure of a Bailu installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.bailu`,
  `~/.bailu/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Bailu itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the bailu coding agent.
...
For example, a report showing that malicious contents written to a trusted Bailu
configuration file cause Bailu to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Bailu.  For exposed-secret reports,
