...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@bailu/coding-agent";
import { Container, hyperlink, Text } from "@bailu/tui";
...
async function fetchAdvisoryMetadata(bailu: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await bailu.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(bailu, cwd, target);
	}
	...
	try {
		const result = await bailu.exec("gh", args);
...
export default function promptUrlWidgetExtension(bailu: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = bailu.getSessionName()?.trim();
		if (!currentName) {
			bailu.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(bailu, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	bailu.on("session_switch", async (_event, ctx) => {
