...
- Removed coding-agent environment-variable defaults from bailu-tui. Applications must configure the hardware cursor and clear-on-shrink behavior through the renderer constructor and `setClearOnShrink()`. `BAILU_DEBUG_REDRAW` is now `BAILU_TUI_DEBUG_REDRAW`; debug and crash log filenames now use the `bailu-tui-` prefix. When no log directory is supplied, redraw logging is disabled and crash dumps are written to the OS temp directory. ([#8699](https://github.com/earendil-works/pi/pull/8699) by [@geraschenko](https://github.com/geraschenko))
...
- Fixed debug and crash logs to use the configured TUI log directory, including `BAILU_CODING_AGENT_DIR`, instead of always writing under `~/.bailu/agent` ([#6958](https://github.com/earendil-works/pi/pull/6958) by [@davidbrai](https://github.com/davidbrai)).
...
- Added `BAILU_DEBUG_REDRAW=1` env var for debugging full redraws (logs triggers to `~/.bailu/agent/bailu-debug.log`)

### Changed

- Terminal height changes no longer trigger full redraws, reducing flicker on resize
- `clearOnShrink` now defaults to `false` (use `BAILU_CLEAR_ON_SHRINK=1` or `setClearOnShrink(true)` to enable)
...
- Added `BAILU_TUI_WRITE_LOG` environment variable to capture raw ANSI output for debugging
