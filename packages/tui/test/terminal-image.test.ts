...
describe("detectCapabilities", () => {
	...
	it("applies environment overrides", () => {
		assert.deepStrictEqual(
			withEnv({ BAILU_HYPERLINKS: "1", BAILU_IMAGE_PROTOCOL: "kitty", BAILU_TRUE_COLOR: "1" }, () =>
				detectCapabilities(),
			),
		...
		assert.deepStrictEqual(
			withEnv(
				{ TERM_PROGRAM: "iterm.app", BAILU_HYPERLINKS: "0", BAILU_IMAGE_PROTOCOL: "none", BAILU_TRUE_COLOR: "0" },
				() => detectCapabilities(),
			),
			...
			withEnv(
				{
					...
					BAILU_HYPERLINKS: "auto",
					BAILU_IMAGE_PROTOCOL: "auto",
					BAILU_TRUE_COLOR: "auto",
				},
				() => detectCapabilities(),
			),
			{ images: "kitty", trueColor: true, hyperlinks: true },
	...
	it("bypasses the tmux probe when hyperlinks are overridden", () => {
		let probed = false;
		const caps = withEnv(
			{ TMUX: "/tmp/tmux-1000/default,1234,0", BAILU_HYPERLINKS: "1", BAILU_IMAGE_PROTOCOL: "kitty" },
			() =>
				detectCapabilities(() => {
					probed = true;
					return false;
...
describe("imageFallback", () => {
	it("shortens home-prefixed absolute paths without hyperlinks", () => {
		setCapabilities({ images: null, trueColor: false, hyperlinks: false });
		try {
			const abs = join(homedir(), ".bailu", "agent", "shot.png");
			const result = imageFallback("image/png", { widthPx: 1280, heightPx: 720 }, abs);
			assert.strictEqual(result, "[Image: ~/.bailu/agent/shot.png [image/png] 1280x720]");
		} finally {
			resetCapabilitiesCache();
		}
	});
	...
	it("wraps shortened absolute paths in OSC 8 file links when hyperlinks are enabled", () => {
		...
		try {
			...
			assert.strictEqual(visible, "[Image: ~/.bailu/agent/shot.png [image/png] 10x10]");
		} finally {
			resetCapabilitiesCache();
		}
	});
