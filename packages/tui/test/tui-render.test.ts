...
describe("TUI debug logging", () => {
	it("writes redraw logs to the provided directory", async () => {
		const logDir = mkdtempSync(join(tmpdir(), "bailu-tui-log-"));
		try {
			await withEnv({ BAILU_TUI_DEBUG_REDRAW: "1" }, async () => {
				const terminal = new VirtualTerminal(40, 10);
				...
				assert.match(readFileSync(join(logDir, "bailu-tui-debug.log"), "utf-8"), /fullRender: first render/);
				tui.stop();
			});
		} finally {
...
describe("TUI crash dump without configured log directory", () => {
	it("writes the crash dump to the OS temp directory instead of a home-directory default", async () => {
		...
		const crashDir = mkdtempSync(join(tmpdir(), "bailu-tui-crash-"));
		const crashLogPath = join(crashDir, "bailu-tui-crash.log");
