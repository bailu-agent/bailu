...
```typescript
import { type TUI, Text, Editor, ProcessTerminal, TuiMainScreen, matchesKey } from "@bailu/tui";
...
```typescript
import { type TUI, TuiAltScreen, TuiMainScreen } from "@bailu/tui";
...
```typescript
import type { TuiMouseEvent, TuiMouseEventResult } from "@bailu/tui";
...
```typescript
import { CombinedAutocompleteProvider } from "@bailu/tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@bailu/tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@bailu/tui";
import type { Component } from "@bailu/tui";
...
```bash
BAILU_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
