import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "@bailu/coding-agent";
import type { Component, EditorTheme, TUI } from "@bailu/tui";
import { truncateToWidth, visibleWidth } from "@bailu/tui";
...
export default function (bailu: ExtensionAPI) {
	...
	bailu.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await bailu.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = bailu.getThinkingLevel();
