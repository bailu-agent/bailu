 ...
 *
 * Runs bailu's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   bailu -e /path/to/bailu/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (bailu: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `bailu ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	bailu.on("session_shutdown", async (_event, ctx) => {
		const activeVm = vm;
		vm = undefined;
		vmStarting = undefined;
		if (!activeVm) return;
	...
	bailu.registerCommand("gondolin", {
	...
	bailu.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
