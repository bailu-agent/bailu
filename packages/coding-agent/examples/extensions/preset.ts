 ...
 * - ~/.bailu/agent/presets.json (global)
 * - <cwd>/.bailu/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `bailu --preset plan` - start with plan preset
...
import type { Api, Model } from "@bailu/ai";
import type { ExtensionAPI, ExtensionContext } from "@bailu/coding-agent";
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@bailu/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@bailu/tui";
...
export default function presetExtension(bailu: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: bailu.getThinkingLevel(),
				tools: bailu.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			bailu.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = bailu.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				bailu.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await bailu.setModel(originalState.model);
				}
				bailu.setThinkingLevel(originalState.thinkingLevel);
				bailu.setActiveTools(originalState.tools);
			} else {
				bailu.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	bailu.registerShortcut(Key.ctrlShift("u"), {
	...
	bailu.on("before_agent_start", async (event) => {
	...
	bailu.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = bailu.getFlag("preset");
	...
	bailu.on("turn_start", async () => {
		if (activePresetName) {
			bailu.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
