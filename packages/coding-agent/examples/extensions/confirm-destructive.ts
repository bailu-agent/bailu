...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@bailu/coding-agent";

export default function (bailu: ExtensionAPI) {
	bailu.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
