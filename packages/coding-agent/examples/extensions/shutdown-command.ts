 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit bailu cleanly.
 */
...
export default function (bailu: ExtensionAPI) {
	// Register a /quit command that cleanly exits bailu
	bailu.registerCommand("quit", {
		description: "Exit bailu cleanly",
	...
	bailu.registerTool({
		...
		description: "Complete a task and exit bailu",
	...
	bailu.registerTool({
		...
		description: "Deploy the application and exit bailu",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await bailu.exec("npm", ["run", "deploy", params.environment], { signal });
