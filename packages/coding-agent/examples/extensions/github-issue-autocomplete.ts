...
async function resolveGitHubRepo(bailu: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await bailu.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (bailu: ExtensionAPI): void {
	bailu.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(bailu, ctx.cwd);
		if (!resolvedRepo.ok) {
