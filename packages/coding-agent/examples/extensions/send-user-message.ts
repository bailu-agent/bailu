 ...
 *
 * Demonstrates bailu.sendUserMessage() for sending user messages from extensions.
 * Unlike bailu.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (bailu: ExtensionAPI) {
	...
	bailu.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			bailu.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	bailu.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				bailu.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	bailu.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				bailu.sendUserMessage(args, { deliverAs: "followUp" });
