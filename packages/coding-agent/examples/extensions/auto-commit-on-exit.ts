...
export default function (bailu: ExtensionAPI) {
	bailu.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await bailu.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[bailu] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await bailu.exec("git", ["add", "-A"]);
		const { code: commitCode } = await bailu.exec("git", ["commit", "-m", commitMessage]);
