 ...
 * to the LLM. Custom entries are stored in the session via bailu.appendEntry() and
 * rendered in interactive mode via bailu.registerEntryRenderer().
 *
...
export default function (bailu: ExtensionAPI) {
	bailu.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	bailu.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			bailu.appendEntry<StatusCardData>("status-card", {
