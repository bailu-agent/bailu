 ...
 *
 * Usage:
 *   bailu -e ./ssh.ts --ssh user@host
 *   bailu -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (bailu: ExtensionAPI) {
	bailu.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	bailu.on("session_start", async (_event, ctx) => {
		...
		const arg = bailu.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
