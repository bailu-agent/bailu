...
export default function (bailu: ExtensionAPI) {
	...
	bailu.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "bailu RPC Demo (new session)" : "bailu RPC Demo");
	...
	bailu.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	bailu.on("turn_end", async (_event, ctx) => {
	...
	bailu.registerCommand("rpc-input", {
	...
	bailu.registerCommand("rpc-editor", {
	...
	bailu.registerCommand("rpc-prefill", {
