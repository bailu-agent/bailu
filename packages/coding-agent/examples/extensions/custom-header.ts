 ...
 * (logo + keybinding hints) with a custom component showing the bailu mascot.
 */

import type { ExtensionAPI, Theme } from "@bailu/coding-agent";
import { VERSION } from "@bailu/coding-agent";

// --- BAILU MASCOT ---
// Based on bailu_mascot.ts - the bailu agent character
function getBailuMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const bailuBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${bailuBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${bailuBlue(BLOCK.repeat(2))}    ${bailuBlue(BLOCK.repeat(2))}`;
...
export default function (bailu: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getBailuMascot(theme);
	...
	bailu.registerCommand("builtin-header", {
