...
The package needs no build script. Bailu asks Chord to discover `src/session.ts` and `src/tui.ts`, builds both entries into its server-owned plugin cache, and sends the TUI artifact to clients.

From the repository root:

```bash
BAILU_EXPERIMENTAL=1 ./bailu-test.sh server \
  -e "$PWD/packages/coding-agent/examples/plugins/bailu-example-plugin"
```
