...
import { Markdown, type MarkdownTheme } from "@bailu/tui";
...
import { formatVersionCheckError, getLatestBailuRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getActiveManagedInstallRoot(): string | undefined {
	...
	// source checkout or another Bailu installation launched from managed Bailu as managed.
...
async function fetchInstallerArtifact(url: string, label: string): Promise<string> {
	const response = await fetch(url, { headers: { "User-Agent": getBailuUserAgent(VERSION) } });
	if (!response.ok) {
...
function verifyManagedRelease(releaseDir: string, expectedVersion: string): void {
	...
	if (result.error || result.status !== 0) {
		...
		throw new Error(`Could not verify managed Bailu ${expectedVersion}: ${reason}`);
	}
	const installedVersion = result.stdout.trim();
	if (installedVersion !== expectedVersion) {
		throw new Error(`Managed Bailu smoke test returned version ${installedVersion}; expected ${expectedVersion}.`);
	}
}
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		...
		if (error instanceof Error && "code" in error && error.code === "ELOCKED") {
			throw new Error("Another managed Bailu update is already running.");
		}
		throw error;
	}
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|bailu] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update bailu, installed packages, or model catalogs.

Options:
  --self                  Update bailu only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update bailu and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall bailu even if the current version is latest

Short forms:
  ${APP_NAME} update                Update bailu only
  ${APP_NAME} update --all          Update bailu and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update bailu             Update bailu only (self works as alias to bailu)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "bailu";
			if (sourceIsSelf) {
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestBailuRelease>>;
	try {
		latestRelease = await getLatestBailuRelease(VERSION, { retry: true });
	} catch (error: unknown) {
