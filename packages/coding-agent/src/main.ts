...
import { type ImageContent, modelsAreEqual } from "@bailu/ai";
import { setCapabilityOverrides } from "@bailu/tui";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.BAILU_OFFLINE);
	if (offlineMode) {
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `bailu update` drain naturally instead.
	...
	// bailu reads user-authored themes, so it opts into full validation before any theme loads.
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.BAILU_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: BAILU_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
