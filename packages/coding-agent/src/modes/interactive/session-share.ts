...
import { DEFAULT_RADIUS_GATEWAY } from "@bailu/ai/providers/radius-config";
import { type Container, type EditorComponent, hyperlink, type TUI } from "@bailu/tui";
...
export async function shareSession(context: SessionShareContext): Promise<void> {
	const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bailu-share-"));
...
async function tryShareViaRadius(tmpFile: string, context: SessionShareContext): Promise<boolean> {
	...
	try {
		...
		url.searchParams.set("title", "Bailu session");
