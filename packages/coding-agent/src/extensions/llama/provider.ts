...
import { stream, streamSimple } from "@bailu/ai/compat";
...
function toBailuModel(model: LlamaModelInfo, serverUrl: string): Model<"openai-completions"> {
...
export function createLlamaProvider(): LlamaProviderController {
	...
	const setCatalog = (
		...
		models = catalog
			...
			.map((model) => toBailuModel(model, serverUrl));
	};
