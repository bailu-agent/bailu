...
export default function llamaExtension(bailu: ExtensionAPI): void {
	const provider = createLlamaProvider();
	bailu.registerProvider(provider.provider);
	...
	const syncCatalog = async (
		...
		const result = await ctx.modelRegistry.refresh({
			...
			// /llama already contacted the configured llama.cpp server, so keep this refresh live even in BAILU_OFFLINE.
			allowNetwork: true,
			signal,
		});
