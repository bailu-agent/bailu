import type { ByteTransport, ByteTransportFactory, ByteTransportHandlers, Client } from "@bailu/client";
import { DEFAULT_MAX_FRAME_LENGTH, type ServerId } from "@bailu/protocol";
import type { Server } from "@bailu/server";
import { WebSocket } from "undici";
import type { RadiusRelayAuthResolver } from "./radius-auth.ts";

export const RADIUS_RELAY_HOST_SUBPROTOCOL = "bailu-session-relay.host.v1";
export const RADIUS_RELAY_CLIENT_SUBPROTOCOL = "bailu-session-relay.client.v1";
...
export class RadiusRelayHost {
	...
	async close(): Promise<void> {
		...
		this.#writer?.close();
		this.#socket?.close(1000, "Bailu server stopped");
...
class RadiusClientByteTransport implements ByteTransport {
	...
	close(): void {
		...
		this.#socket.close(1000, "Bailu client closed");
	}
