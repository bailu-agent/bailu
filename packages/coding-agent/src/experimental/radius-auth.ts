...
import { DEFAULT_RADIUS_GATEWAY, normalizeRadiusGatewayUrl } from "@bailu/ai/providers/radius-config";
...
export const ENV_RADIUS_GATEWAY = "BAILU_RADIUS_GATEWAY";
...
export class RadiusRelayAuthResolver {
	...
	async resolve(options: {
		...
		if (process.env.BAILU_OFFLINE !== undefined) {
		...
		if (options.required) {
			throw new Error("Radius authentication is required; start Bailu and run /login radius, then retry");
		}
		return undefined;
	}
