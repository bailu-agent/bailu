...
import { BACKGROUND_CONTEXT, type JsonlSessionMetadata, JsonlSessionRepo, TODO_CONTEXT } from "@bailu/agent-core";
import { NodeExecutionEnv } from "@bailu/agent-core/node";
import type { Context } from "@bailu/chord";
import type { FacetBundleArtifact } from "@bailu/chord/node";
import { Client, ServerError as ClientServerError, DisconnectedError } from "@bailu/client";
import { createUnixTransportFactory, type UnixServerRoute } from "@bailu/client/unix";
import { isServerId, type ServerId } from "@bailu/protocol";
import {
...
import { createUnixServer, getUnixSocketPath } from "@bailu/server/unix";
...
export const ENV_SERVER_DIR = "BAILU_SERVER_DIR";
export const ENV_SERVER_ID = "BAILU_SERVER_ID";

export function resolveServerDirectory(directory?: string): string {
	return resolvePath(directory ?? process.env[ENV_SERVER_DIR] ?? join(homedir(), ".bailu", "server"));
}
...
/** Ensure the selected logical server is reachable, launching the current Bailu installation if needed. */
...
export interface StartServerOptions {
	/** Server profile and socket directory. Defaults to BAILU_SERVER_DIR or ~/.bailu/server. */
	readonly directory?: string;
	/** Logical service ID. Defaults to BAILU_SERVER_ID or the directory's default-server-id. */
	readonly serverId?: ServerId;
