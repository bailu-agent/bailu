...
import type { AgentMessage, Entry, LaneSnapshot } from "@bailu/agent-core";
import type { AssistantMessage, AuthEvent } from "@bailu/ai";
import {
	type Component,
	Container,
	type Focusable,
...
function runLogin(
	...
	if (!account.interactive) {
		dialog.showInfo(`${account.methodName ?? "Authentication"} is configured outside bailu.`, [], true);
