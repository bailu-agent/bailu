import type { AuthResult } from "@bailu/ai";
...
export function printAuthCommandHelp(): void {
	console.log(`Usage:
  bailu auth print-api-key [--provider <provider>] [--model <model>]
  bailu auth print-bearer-token [--provider <provider>] [--model <model>] [--min-expiry <duration>]
  bailu auth check [--provider <provider>] [--model <model>] [--json] [--credentials] [--no-refresh]
