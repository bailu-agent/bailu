...
import { type BailuManifest, readBailuManifest } from "./bailu-manifest.ts";
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.BAILU_OFFLINE;
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside bailu and resolve bailu APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @bailu/* peers. Stale auto-installed bailu peers can otherwise block updates.
	...
	private getGitUpdateMarkerPath(targetDir: string): string {
		return join(dirname(targetDir), `.${basename(targetDir)}.bailu-update-incomplete`);
	}
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "bailu-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = readBailuManifest(join(packageRoot, "package.json"));
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof BailuManifest];
	...
	private addAutoDiscoveredResources(
		...
		// User extensions from ~/.bailu/agent/
		addResources(
			"extensions",
