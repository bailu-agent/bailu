...
import type { AgentTool } from "@bailu/agent-core";
...
export const bashToolSystemPromptContribution = {
	...
	guidelines: ["You can inspect BAILU_* environment variables for current model and session details."],
} as const;
...
/**
 * Create bash operations using bailu's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want bailu's
...
function resolveSpawnContext(
	...
	delete env.BAILU_SESSION_ID;
	delete env.BAILU_SESSION_FILE;
	delete env.BAILU_PROVIDER;
	delete env.BAILU_MODEL;
	delete env.BAILU_REASONING_LEVEL;
	if (exposeSessionEnvironment && ctx) {
		const model = ctx.model;
		env.BAILU_SESSION_ID = ctx.sessionManager.getSessionId();
		const sessionFile = ctx.sessionManager.getSessionFile();
		if (sessionFile) env.BAILU_SESSION_FILE = sessionFile;
		if (model) {
			env.BAILU_PROVIDER = model.provider;
			env.BAILU_MODEL = model.id;
		}
		if (ctx.thinkingLevel) env.BAILU_REASONING_LEVEL = ctx.thinkingLevel;
	}
...
export interface BashToolOptions {
	...
	shellPath?: string;
	/** Expose current Bailu session metadata as BAILU_* environment variables. Default: true */
	exposeSessionEnvironment?: boolean;
