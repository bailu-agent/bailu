...
import * as builtinProviderCatalog from "@bailu/ai/providers/all";
...
/** Configured bailu-ai Models collection used by coding-agent and SDK consumers. */
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const runtime = new ModelRuntime(
			...
			modelsStore,
			providers,
			process.env.BAILU_OFFLINE === undefined,
		);
	...
	async refresh(options: ModelsRefreshOptions = {}): Promise<ModelsRefreshResult> {
		...
		// Published bailu-ai builds before ModelsStore returned void and accepted a provider ID.
