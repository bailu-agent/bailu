...
import * as _bundledBailuAgentCore from "@bailu/agent-core";
import type { Provider } from "@bailu/ai";
import * as _bundledBailuAiCompat from "@bailu/ai/compat";
import * as _bundledBailuAiOauth from "@bailu/ai/oauth";
import * as _bundledBailuAiProviders from "@bailu/ai/providers/all";
import type { KeyId } from "@bailu/tui";
import * as _bundledBailuTui from "@bailu/tui";
...
// avoiding a circular dependency. Extensions can import from @bailu/coding-agent.
import * as _bundledBailuCodingAgent from "../../index.ts";
import { resolvePath } from "../../utils/paths.ts";
import { readBailuManifest } from "../bailu-manifest.ts";
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@bailu/agent-core": _bundledBailuAgentCore,
	"@bailu/tui": _bundledBailuTui,
	// Extensions resolve the bailu-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	"@bailu/ai": _bundledBailuAiCompat,
	"@bailu/ai/compat": _bundledBailuAiCompat,
	"@bailu/ai/oauth": _bundledBailuAiOauth,
	"@bailu/ai/providers/all": _bundledBailuAiProviders,
	"@bailu/coding-agent": _bundledBailuCodingAgent,
	"@mariozechner/pi-agent-core": _bundledBailuAgentCore,
	"@mariozechner/pi-tui": _bundledBailuTui,
	"@mariozechner/pi-ai": _bundledBailuAiCompat,
	"@mariozechner/pi-ai/compat": _bundledBailuAiCompat,
	"@mariozechner/pi-ai/oauth": _bundledBailuAiOauth,
	"@mariozechner/pi-ai/providers/all": _bundledBailuAiProviders,
	"@mariozechner/pi-coding-agent": _bundledBailuCodingAgent,
};
...
function getAliases(): Record<string, string> {
	...
	const bailuCodingAgentEntry = packageIndex;
	const bailuAgentCoreEntry = resolveWorkspaceOrImport("agent/dist/index.js", "@bailu/agent-core");
	const bailuTuiEntry = resolveWorkspaceOrImport("tui/dist/index.js", "@bailu/tui");
	// Extensions resolve the bailu-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	const bailuAiCompatEntry = resolveWorkspaceOrImport("ai/dist/compat.js", "@bailu/ai/compat");
	const bailuAiOauthEntry = resolveWorkspaceOrImport("ai/dist/oauth.js", "@bailu/ai/oauth");
	const bailuAiProvidersEntry = resolveWorkspaceOrImport("ai/dist/providers/all.js", "@bailu/ai/providers/all");

	_aliases = {
		"@bailu/coding-agent": bailuCodingAgentEntry,
		"@bailu/agent-core": bailuAgentCoreEntry,
		"@bailu/tui": bailuTuiEntry,
		"@bailu/ai/providers/all": bailuAiProvidersEntry,
		"@bailu/ai/compat": bailuAiCompatEntry,
		"@bailu/ai/oauth": bailuAiOauthEntry,
		"@bailu/ai": bailuAiCompatEntry,
		"@mariozechner/pi-coding-agent": bailuCodingAgentEntry,
		"@mariozechner/pi-agent-core": bailuAgentCoreEntry,
		"@mariozechner/pi-tui": bailuTuiEntry,
		"@mariozechner/pi-ai/providers/all": bailuAiProvidersEntry,
		"@mariozechner/pi-ai/compat": bailuAiCompatEntry,
		"@mariozechner/pi-ai/oauth": bailuAiOauthEntry,
		"@mariozechner/pi-ai": bailuAiCompatEntry,
...
export function createExtensionRuntime(): ExtensionRuntime {
	...
	const runtime: ExtensionRuntime = {
		...
		invalidate: (message) => {
			...
			state.staleMessage =
				message ??
				"This extension ctx is stale after session replacement or reload. Do not use a captured bailu or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload(). For newSession, fork, and switchSession, move post-replacement work into withSession and use the ctx passed to withSession. For reload, do not use the old ctx after await ctx.reload().";
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readBailuManifest(packageJsonPath);
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with bailu manifest or index.ts
