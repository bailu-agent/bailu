...
Bailu supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with bailu; configured providers may refresh newer catalogs and cache them in `~/.bailu/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.bailu/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Radius is a dynamic `bailu-messages` gateway. `/login radius` stores OAuth tokens in `auth.json`; the gateway catalog is refreshed independently and cached in `models-store.json`. Custom Radius gateways can be declared in `models.json` with `"oauth": "radius"` and a gateway `baseUrl`.
...
Store credentials in `~/.bailu/agent/auth.json`:
...
API key credentials can also include provider-scoped environment values. These values are used before process environment variables when resolving the credential key, provider/model headers, and provider configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `BAILU_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
Use this when bailu should use different provider settings than the project shell environment.
...
```bash
bailu --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
bailu --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
bailu --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal bailu usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
bailu --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Bailu automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
...
Bailu supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage loaded models with `/llama`, and select a loaded model with `/model`.
