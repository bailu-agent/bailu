> bailu can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to bailu's agent capabilities. Use it to embed bailu in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @bailu/coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@bailu/coding-agent";
...
The `Agent` class (from `@bailu/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.bailu/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.bailu/extensions/`)
- Project skills:
  - `.bailu/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.bailu/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.bailu/agent/skills/`)
  - `~/.agents/skills/`
...
Remote catalogs are persisted locally so later runtimes can restore them without a network request. The default file is `~/.bailu/agent/models-store.json`; set `modelsStorePath` to choose another location, or inject `modelsStore` to control persistence. Network refreshes are throttled to once per provider every four hours unless forced. To force an immediate refresh, call `await modelRuntime.refresh({ allowNetwork: true, force: true, signal })`. Setting `BAILU_OFFLINE` disables model network access.
...
```typescript
import { InMemoryCredentialStore } from "@bailu/ai";
import { createAgentSession, ModelRuntime } from "@bailu/coding-agent";

// Default: uses ~/.bailu/agent/auth.json and ~/.bailu/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any bailu-ai CredentialStore
...
The `edit` tool returns `details.diff` for Bailu's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@bailu/coding-agent";
...
import { createAgentSession, defineTool } from "@bailu/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@bailu/coding-agent";
...
1. Global: `~/.bailu/agent/settings.json`
2. Project: `<cwd>/.bailu/settings.json`
...
// Auth and Models
ModelRuntime // implements bailu-ai Models and owns credential storage
