> bailu can create themes. Ask it to build one for your setup.
...
Bailu loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.bailu/agent/themes/*.json`
- Project: `.bailu/themes/*.json` (only after the project is trusted)
...
On first run, bailu detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.bailu/agent/themes
vim ~/.bailu/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, bailu reloads it automatically for immediate visual feedback.
...
Bailu uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, bailu falls back to the nearest approximation.
