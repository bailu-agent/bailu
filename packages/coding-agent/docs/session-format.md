...
```
~/.bailu/agent/sessions/--<path>--/<timestamp>_<session-id>.jsonl
```

By default, `<session-id>` is a UUID. Callers can supply a custom ID through the SDK or `--session-id`. For `<path>`, Bailu removes the leading path separator and replaces `/`, `\\`, and `:` with `-`.

## Deleting Sessions

Sessions can be removed by deleting their `.jsonl` files under `~/.bailu/agent/sessions/`.

Bailu also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, bailu uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@bailu/coding-agent/dist/` and `node_modules/@bailu/ai/dist/`.
...
### Base Message Types (from bailu-ai)
...
`firstKeptEntryId` is required. It identifies the first entry retained from before the compaction entry. When rebuilding context, Bailu replaces older summarized entries with the compaction summary and keeps the range beginning at this entry.

Optional fields:
- `usage`: LLM usage from generating the summary; included in session token and cost totals
- `details`: Implementation-specific data (e.g., `{ readFiles: string[], modifiedFiles: string[] }` for default, or custom data for extensions)
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if bailu-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `bailu.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `bailu.setSessionName()` in extensions.
