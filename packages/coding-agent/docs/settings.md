...
Bailu uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.bailu/agent/settings.json` | Global (all projects) |
| `.bailu/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`BAILU_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so bailu resumes after the editor exits:
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Bailu sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.bailu/agent/npm/`; project-scoped npm packages install under `.bailu/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.

### Tools

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `defaultTools` | string[] | - | Built-in tools enabled initially. When omitted, Bailu uses its standard defaults |
...
```json
{ "sessionDir": ".bailu/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `BAILU_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.bailu/agent/settings.json` resolve relative to `~/.bailu/agent`. Paths in `.bailu/settings.json` resolve relative to `.bailu`. Absolute paths and `~` are supported.
...
```json
{
  "packages": ["bailu-skills", "@org/my-extension"]
}
```
...
```json
{
  ...
  "packages": ["bailu-skills"]
}
```

## Project Overrides

Project settings (`.bailu/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.bailu/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .bailu/settings.json (project)
{
