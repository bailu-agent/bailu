> bailu can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend bailu's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.bailu/agent/extensions/` (global) or `.bailu/extensions/` (project-local) for auto-discovery. Use `bailu -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `bailu.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `bailu.registerCommand()`
- **Session persistence** - Store state that survives restarts via `bailu.appendEntry()`
...
Create `~/.bailu/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.bailu/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.bailu/agent/extensions/*.ts` | Global (all projects) |
| `~/.bailu/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.bailu/extensions/*.ts` | Project-local |
| `.bailu/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as bailu packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@bailu/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@bailu/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@bailu/tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed bailu packages installed with `bailu install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (bailu: ExtensionAPI) {
  // Subscribe to events
  bailu.on("event_name", async (event, ctx) => {
  ...
  bailu.registerTool({ ... });
  bailu.registerCommand("name", { ... });
  bailu.registerShortcut("ctrl+x", { ... });
  bailu.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, bailu awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `bailu.registerProvider()` are flushed.
...
export default async function (bailu: ExtensionAPI) {
  ...
  bailu.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `bailu --list-models`.
...
thinking level changes (settings, keybinding, bailu.setThinkingLevel())
...
Fired before bailu decides whether to trust a project with dynamic configs (`.bailu` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
bailu.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether bailu asks, trusts, or declines by default.
...
```typescript
bailu.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
bailu.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `bailu.setSessionName()`.

```typescript
bailu.on("session_info_changed", async (event, ctx) => {
...
```typescript
bailu.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, bailu emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
bailu.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, bailu emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
bailu.on("session_compact", async (event, ctx) => {
...
bailu.on("session_tree", async (event, ctx) => {
...
```typescript
bailu.on("session_shutdown", async (event, ctx) => {
...
```typescript
bailu.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Bailu uses to build the system prompt. This lets you inspect what Bailu has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Bailu may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Bailu will not continue running automatically.

```typescript
bailu.on("agent_start", async (_event, ctx) => {});

bailu.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

bailu.on("agent_settled", async (_event, ctx) => {
...
```typescript
bailu.on("ui_prompt_start", async (event, ctx) => {
  // event.reason === "ui_prompt"
  // event.kind: "select" | "confirm" | "input" | "editor" | "custom"
  // event.title: prompt title when available
});

bailu.on("ui_prompt_end", async (event, ctx) => {
  // Bailu is no longer waiting on that UI prompt span.
});
```
...
```typescript
bailu.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

bailu.on("turn_end", async (event, ctx) => {
...
```typescript
bailu.on("message_start", async (event, ctx) => {
  // event.message
});

bailu.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

bailu.on("message_end", async (event, ctx) => {
...
```typescript
bailu.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

bailu.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

bailu.on("tool_execution_end", async (event, ctx) => {
...
```typescript
bailu.on("context", async (event, ctx) => {
...
```typescript
bailu.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header bailu adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Bailu's system prompt string rather than the final serialized provider payload.

```typescript
bailu.on("before_provider_request", (event, ctx) => {
...
```typescript
bailu.on("after_provider_response", (event, ctx) => {
...
```typescript
bailu.on("model_select", async (event, ctx) => {
...
```typescript
bailu.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `bailu.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, bailu waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@bailu/coding-agent";
...
```typescript
import { isBashToolResult } from "@bailu/coding-agent";

bailu.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@bailu/coding-agent";

bailu.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap bailu's built-in local bash backend
...
```typescript
bailu.on("input", async (event, ctx) => {
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.bailu` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@bailu/coding-agent";
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective bailu-ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
Use `ctx.modelRegistry.streamSimple(model, context, options)` for provider-neutral options such as `reasoning`, or `stream()` for API-specific options. Both use configured providers and resolve authentication, including for providers registered with `bailu.registerProvider()`. Use these instead of `bailu-ai/compat` streaming functions, which cannot see extension provider registrations.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while bailu is idle.
...
Control flow helpers. `ctx.isIdle()` is false while Bailu is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of bailu.
...
Returns Bailu's current system prompt string.
...
```typescript
bailu.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Bailu currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `bailu` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@bailu/coding-agent";

bailu.registerCommand("switch", {
...
- Captured old `bailu` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
bailu.registerCommand("reload-runtime", {
...
export default function (bailu: ExtensionAPI) {
  ...
  bailu.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      bailu.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### bailu.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### bailu.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`bailu.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `bailu.getAllTools()` and are callable by the LLM without `/reload`.

Use `bailu.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@bailu/ai";
...
### bailu.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`bailu.appendEntry()`](#bailuappendentrycustomtype-data) with [`bailu.registerEntryRenderer()`](#bailuregisterentryrenderercustomtype-renderer).
...
### bailu.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
bailu.sendUserMessage("What is 2+2?");

// With content array (text + images)
bailu.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
bailu.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
bailu.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
bailu.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
### bailu.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `bailu.registerEntryRenderer()`.

```typescript
bailu.appendEntry("my-state", { count: 42 });
bailu.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### bailu.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
bailu.setSessionName("Refactor auth module");
```

### bailu.getSessionName()

Get the current session name, if set.

```typescript
const name = bailu.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### bailu.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
bailu.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
bailu.setLabel(entryId, undefined);
...
### bailu.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, bailu keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@bailu/tui";

bailu.registerCommand("deploy", {
...
```typescript
const commands = bailu.getCommands();
...
### bailu.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `bailu.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### bailu.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, Bailu renders the transformed content with its built-in renderer.
...
```typescript
bailu.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
  if (isStreaming || messageType === "assistant-thinking") return markdown;
  return markdown.replaceAll("-->", "→");
});
```

If a transformer throws, Bailu keeps the Markdown produced so far and continues with the next transformer. The hook is display-only: the original message remains unchanged in the session and model context. It runs for new user messages, assistant streaming updates, restored session messages, and terminal width changes, so transformers should remain synchronous and inexpensive.

### bailu.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `bailu.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@bailu/tui";

bailu.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### bailu.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
bailu.registerShortcut("ctrl+shift+p", {
...
### bailu.registerFlag(name, options)

Register a CLI flag.

```typescript
bailu.registerFlag("plan", {
...
// Check value
if (bailu.getFlag("plan")) {
  // Plan mode enabled
}
```

### bailu.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await bailu.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### bailu.getActiveTools() / bailu.getAllTools() / bailu.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `bailu.getActiveTools()` returns the active tool names as `string[]`; `bailu.getAllTools()` returns metadata for all configured tools.

```typescript
const active = bailu.getActiveTools(); // ["read", "bash", ...]
const all = bailu.getAllTools();
// all = [{
//   name: "read",
...
bailu.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
bailu.setActiveTools(["read", "bash"]); // Switch to read-only
```

`bailu.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await bailu.setModel(model);
  if (!success) {
...
### bailu.getThinkingLevel() / bailu.setThinkingLevel(level)
...
`bailu.setThinkingLevel()` changes the thinking level for the current session. The change is recorded in session history and restored when that session is resumed, but it does not change the configured default used by new sessions.

```typescript
const current = bailu.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
bailu.setThinkingLevel("high");
```
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@bailu/ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@bailu/ai";
...
bailu.registerProvider(provider);

// Register a new provider with custom models
bailu.registerProvider("my-proxy", {
  name: "My Proxy",
...
bailu.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
bailu.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
bailu.registerProvider("corporate-ai", {
...
The object form accepts a complete bailu-ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
bailu.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    bailu.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `bailu.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `bailu.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@bailu/coding-agent";
...
bailu.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via bailu.exec (captured from extension closure)
    const result = await bailu.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Bailu persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@bailu/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when bailu resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
bailu -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
bailu --no-builtin-tools -e ./my-extension.ts
```
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@bailu/coding-agent";
...
For `user_bash`, extensions can reuse bailu's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
`createBashTool()` and `createPowerShellTool()` expose the current session to commands through `BAILU_SESSION_ID`, `BAILU_SESSION_FILE`, `BAILU_PROVIDER`, `BAILU_MODEL`, and `BAILU_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
import {
  ...
  truncateLine,      // Truncate a single line to maxChars with ellipsis
...
export default function (bailu: ExtensionAPI) {
  ...
  bailu.registerTool({ name: "db_connect", ... });
  bailu.registerTool({ name: "db_query", ... });
  bailu.registerTool({ name: "db_close", ... });

  bailu.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
```typescript
import { keyHint } from "@bailu/coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `bailu.setActiveTools()` during execution. Bailu detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `bailu.registerTool()` so it appears in `bailu.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `bailu.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Bailu records which tools were added on the loader's tool result.
5. Before the next model response, Bailu exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `bailu.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Bailu adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Bailu sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Bailu also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (bailu: ExtensionAPI) {
  ...
  bailu.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = bailu.getAllTools()
      ...
      bailu.setActiveTools([...new Set([...active, ...added])]);
  ...
  bailu.on("session_start", () => {
    ...
    const initialTools = bailu.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    bailu.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("bailu - my-project");
...
```typescript
import { Text, Component } from "@bailu/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@bailu/coding-agent";
import { matchesKey } from "@bailu/tui";
...
bailu.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `bailu.sendMessage()`:
...
```typescript
bailu.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

bailu.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@bailu/coding-agent";

// Highlight code with explicit language (returns an array of rendered lines)
...
| `event-bus.ts` | Inter-extension events | `bailu.events` |
| **Session Metadata** |||
