...
**Note for Node.js/TypeScript users**: If you're building a Node.js application, consider using `AgentSession` directly from `@bailu/coding-agent` instead of spawning a subprocess. See [`src/core/agent-session.ts`](../src/core/agent-session.ts) for the API. For a subprocess-based TypeScript client, see [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts).

## Starting RPC Mode

```bash
bailu --mode rpc [options]
```
...
```json
{
  ...
  "data": {
    ...
    "cancelled": false,
    "truncated": true,
    "fullOutputPath": "/tmp/bailu-bash-abc123.log"
  }
}
```
...
The current session name is available via `get_state` in the `sessionName` field. To set the initial name when starting RPC mode, pass `--name <name>` or `-n <name>` to the `bailu --mode rpc` process.
...
Emitted after the full session-level run settles. At this point Bailu will not continue automatically through retry, compaction retry, or queued follow-up messages.
...
const agent = spawn("bailu", ["--mode", "rpc", "--no-session"]);
