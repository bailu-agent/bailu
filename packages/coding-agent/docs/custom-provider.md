...
Extensions can register custom model providers via `bailu.registerProvider()`. This enables:
...
Extensions can register either a complete bailu-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Bailu composes `models.json` overrides above registered native providers.

```typescript
import { createProvider } from "@bailu/ai";
...
export default function (bailu: ExtensionAPI) {
  bailu.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  bailu.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. bailu waits for the factory before startup continues, so the provider is available during interactive startup and to `bailu --list-models`.
...
Use `bailu.unregisterProvider(name)` to remove a provider that was previously registered via `bailu.registerProvider(name, ...)`:
...
// Later, remove it
bailu.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map bailu levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
bailu.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@bailu/ai";
...
Credentials are persisted in `~/.bailu/agent/auth.json`:
...
When a request exceeds the model's context window, bailu can recover automatically by compacting the conversation and retrying. This recovery only kicks in if bailu recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of bailu's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/earendil-works/pi/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message bailu does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase bailu recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (bailu: ExtensionAPI) {
  ...
  bailu.on("message_end", (event, ctx) => {
...
`message_end` runs before bailu tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what bailu checks. With this in place, bailu will:
...
- Match a provider-specific pattern, not bailu's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of bailu's normal retry-with-backoff path.
...
interface ProviderModelConfig {
  ...
  /** Maps bailu thinking levels to provider/model-specific values; null marks a level unsupported. */
