> bailu can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@bailu/tui`](https://github.com/earendil-works/pi/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@bailu/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the renderer's `showHardwareCursor` constructor argument or `setShowHardwareCursor(true)`. Bailu also maps `BAILU_HARDWARE_CURSOR=1` to this setting before it creates its renderer. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@bailu/tui";
...
Import from `@bailu/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@bailu/tui";
```
...
```typescript
import { matchesKey, Key } from "@bailu/tui";
...
```typescript
import { MouseRegion } from "@bailu/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@bailu/tui";
...
```typescript
import { getMarkdownTheme } from "@bailu/coding-agent";
import { Markdown } from "@bailu/tui";
...
Set `BAILU_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
BAILU_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@bailu/tui` with `DynamicBorder` for framing.
...
For toggling multiple settings. Use `SettingsList` from `@bailu/tui` with `getSettingsListTheme()`.
...
Customize the inline working indicator shown while bailu is streaming a response.
...
// Restore bailu's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@bailu/tui";
