...
import {
	checkForNewBailuVersion,
	comparePackageVersions,
	formatVersionCheckError,
	getLatestBailuRelease,
	getLatestBailuVersion,
	isNewerPackageVersion,
} from "../src/utils/version-check.ts";
import { allowNetwork } from "./test-network-env.ts";

const originalSkipVersionCheck = process.env.BAILU_SKIP_VERSION_CHECK;
...
afterEach(() => {
	...
	if (originalSkipVersionCheck === undefined) {
		...
		process.env.BAILU_SKIP_VERSION_CHECK = originalSkipVersionCheck;
	}
});
...
describe("version checks", () => {
	...
	it("returns only newer versions", async () => {
		...
		await expect(checkForNewBailuVersion("1.2.3")).resolves.toBeUndefined();
	...
	it("retries a transient version request when explicitly requested", async () => {
		...
		await expect(getLatestBailuRelease("1.2.3", { retry: true })).resolves.toEqual({ version: "1.2.4" });
	...
	it("returns update notes from the version check api", async () => {
		...
		await expect(getLatestBailuRelease("1.2.3")).resolves.toEqual({ note: "**Read this**", version: "1.2.4" });
	});
