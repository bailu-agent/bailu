...
import { BACKGROUND_CONTEXT } from "@bailu/chord/context";
import { readFacetBundleManifest } from "@bailu/chord/node";
...
describe("server-selected presentation facets", () => {
	...
	test("restores plugin package selections for later server generations", async () => {
		const directory = await mkdtemp("/tmp/bailu-facets-profile-");
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		// This fixture owns a server, whose socket path is derived as
		// <directory>/server/server-<uuid>-<nonce>.sock. The publish bind links into that
		// path, and macOS refuses to connect above 104 characters, so the prefix stays short.
		const directory = await mkdtemp("/tmp/bailu-facets-");
		directories.add(directory);
		const serverId = randomUUID();
		const packagePath = join(directory, "bailu-example-plugin");
		await mkdir(join(packagePath, "src"), { recursive: true });
		await writeFile(
			join(packagePath, "package.json"),
			`${JSON.stringify({
				name: "@bailu/test-plugin",
				version: "1.0.0",
				peerDependencies: {
					"@bailu/chord": "^0.84.4",
					"@bailu/coding-agent": "^0.84.4",
				},
			})}\n`,
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@bailu/chord"; import { SlashCommands } from "@bailu/coding-agent/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		expect(plugin.manifestPath).toMatch(
			new RegExp(`/plugin-builds/${serverId}/bailu-example-plugin-[a-f0-9]{12}/chord-facets\\.json$`, "u"),
		);
		expect(first[0]?.plugin).toEqual({ id: "@bailu/test-plugin", version: "1.0.0" });
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@bailu/chord"; import { SlashCommands } from "@bailu/coding-agent/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					name: "@bailu/second-test-plugin",
					version: "1.0.0",
					peerDependencies: { "@bailu/chord": "^0.84.4" },
				})}\n`,
			),
			writeFile(
				join(secondPackagePath, "src", "tui.ts"),
				'import { defineFacet } from "@bailu/chord"; export default defineFacet({ id: "second-built", setup() {} });\n',
			),
		]);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@bailu/chord"; import { SlashCommands } from "@bailu/coding-agent/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
	...
	test("builds the example plugin package without a package-owned build script", async () => {
		const directory = await mkdtemp("/tmp/bailu-facets-plugin-");
		directories.add(directory);
		const serverId = randomUUID();
		const packagePath = fileURLToPath(new URL("../examples/plugins/bailu-example-plugin", import.meta.url));
		const plugin = createServerPluginPackage(directory, serverId, packagePath);

		const artifacts = await plugin.build();
		const manifest = await readFacetBundleManifest(plugin.manifestPath);
		expect(manifest.plugin).toEqual({ id: "@bailu/example-plugin", version: "1.0.0" });
		...
		expect(loaded.facets.map(({ id }) => id)).toEqual(["@bailu/example-plugin/tui"]);
