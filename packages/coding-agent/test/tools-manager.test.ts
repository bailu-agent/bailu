...
const originalOffline = process.env.BAILU_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.BAILU_OFFLINE;
	else process.env.BAILU_OFFLINE = originalOffline;
	vi.unstubAllGlobals();
});
