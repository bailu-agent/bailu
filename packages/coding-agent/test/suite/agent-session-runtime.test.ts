...
import { fauxAssistantMessage, fauxToolCall, registerFauxProvider } from "@bailu/ai/compat";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const tempDir =
			options?.cwd ?? join(tmpdir(), `bailu-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const runtimeOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(bailu: ExtensionAPI) => {
						bailu.registerProvider(faux.getModel().provider, {
	...
	it("persists message_end assistant replacements to the session manager", async () => {
		const { runtime } = await createRuntimeForTest((bailu: ExtensionAPI) => {
			bailu.on("message_end", (event) => {
	...
	it("settles the active response before session replacement", async () => {
		...
		const { runtime, faux } = await createRuntimeForTest((bailu: ExtensionAPI) => {
			bailu.registerTool({
				name: "block",
				label: "Block",
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtime } = await createRuntimeForTest((bailu: ExtensionAPI) => {
			bailu.on("session_before_switch", (event) => {
				events.push(event);
			});
			bailu.on("session_shutdown", (event) => {
				events.push(event);
			});
			bailu.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("honors session_before_switch cancellation for new and resume", async () => {
		...
		events.length = 0;
		const otherDir = join(tmpdir(), `bailu-runtime-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtime } = await createRuntimeForTest((bailu: ExtensionAPI) => {
			bailu.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
	...
	it("duplicates the current active branch in-memory when forking at the current position", async () => {
		const tempDir = join(
			tmpdir(),
			`bailu-runtime-suite-in-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		mkdirSync(tempDir, { recursive: true });
	...
	it("updates the runtime session cwd on cross-cwd session replacement", async () => {
		const firstDir = join(tmpdir(), `bailu-runtime-cwd-a-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const secondDir = join(tmpdir(), `bailu-runtime-cwd-b-${Date.now()}-${Math.random().toString(36).slice(2)}`);
