import { type AssistantMessage, fauxAssistantMessage, fauxProvider } from "@bailu/ai";
import { getApiProvider } from "@bailu/ai/compat";
import { expect, it } from "vitest";
import { createHarness } from "../harness.ts";

// Regression for #8964: extensions can stream responses from providers registered with bailu.registerProvider().
it.each(["stream", "streamSimple"] as const)(
	...
	async (method) => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(bailu) => {
					bailu.registerProvider(faux.provider.id, {
						api: faux.api,
				...
				(bailu) => {
					bailu.registerCommand("stream-custom", {
