...
describe("issue #8423 extension factory failure", () => {
	it("discards runtime changes and disables the failed API", async () => {
		...
		await loadExtensionFromFactory(
			(bailu) => bailu.registerProvider("working-provider", providerConfig),
			process.cwd(),
			eventBus,
			runtime,
			"<working>",
		...
		await expect(
			loadExtensionFromFactory(
				(bailu) => {
					capturedApi = bailu;
					bailu.events.on("factory-failure", () => {
						eventCalls++;
					});
					bailu.registerFlag("failed-flag", { type: "boolean", default: true });
					flagDuringLoad = bailu.getFlag("failed-flag");
					bailu.unregisterProvider("working-provider");
					bailu.registerProvider("failed-provider", providerConfig);
