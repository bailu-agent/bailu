import { Container } from "@bailu/tui";
...
describe("issue #7829 invalid settings warning", () => {
	...
	it("renders startup diagnostics inside the transcript", async () => {
		...
		const previousOffline = process.env.BAILU_OFFLINE;
		...
		try {
			...
			if (previousOffline === undefined) delete process.env.BAILU_OFFLINE;
			else process.env.BAILU_OFFLINE = previousOffline;
			harness.cleanup();
		}
	});
});
