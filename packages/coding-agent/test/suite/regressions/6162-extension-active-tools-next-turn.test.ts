...
describe("extension active tools next-turn refresh", () => {
	it("applies bailu.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(bailu) => {
				bailu.registerTool({
					...
					execute: async () => {
						bailu.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(bailu) => {
				bailu.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						bailu.setActiveTools([...bailu.getActiveTools(), "after_load"]);
						return {
