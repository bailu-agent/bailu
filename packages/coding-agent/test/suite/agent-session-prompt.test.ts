...
import { fauxAssistantMessage, fauxToolCall, type Model } from "@bailu/ai";
...
describe("AgentSession prompt characterization", () => {
	...
	it("expands skill commands before sending the prompt", async () => {
		const tempDir = join(tmpdir(), `bailu-skill-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("throws when prompted during manual compaction", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(bailu) => {
					bailu.on("session_before_compact", async (event) => {
