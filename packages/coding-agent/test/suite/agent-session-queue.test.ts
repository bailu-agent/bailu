...
import { fauxAssistantMessage, fauxToolCall } from "@bailu/ai";
import type { ExtensionAPI, InputEvent } from "@bailu/coding-agent";
			...
			extensionFactories: [
				(bailu: ExtensionAPI) => {
					bailu.on("agent_end", async () => {
						if (sent) return;
						sent = true;
						bailu.sendUserMessage("conflict report", { deliverAs: "followUp" });
					});
				},
			],
		});
