...
import type { AgentMessage, AgentTool } from "@bailu/agent-core";
import { Agent } from "@bailu/agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@bailu/ai/compat";
import { registerFauxProvider, streamSimple } from "@bailu/ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `bailu-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
