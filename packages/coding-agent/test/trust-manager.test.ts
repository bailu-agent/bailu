...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".bailu", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".bailu", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".bailu", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".bailu"), { recursive: true });
			writeFileSync(join(cwd, ".bailu", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".bailu"), { recursive: true, force: true });
