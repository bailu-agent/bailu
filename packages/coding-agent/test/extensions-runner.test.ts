...
describe("ExtensionRunner", () => {
	...
	beforeEach(async () => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bailu-runner-test-"));
	...
	bailu.on("project_trust", () => ({ trusted: "undecided", remember: true }));
}`,
			);
			fs.writeFileSync(
				decidedPath,
				`export default function(bailu) {
	bailu.on("project_trust", () => ({ trusted: "no", remember: true }));
}`,
			);
	...
	describe("shortcut conflicts", () => {
		...
		it("warns but allows when extension uses non-reserved built-in shortcut", async () => {
			...
			const extCode = `
				export default function(bailu) {
					bailu.registerShortcut("${pasteImageKey}", {
		...
		it("warns when two extensions register same shortcut", async () => {
			...
			const extCode1 = `
				export default function(bailu) {
					bailu.registerShortcut("ctrl+shift+x", {
	...
	describe("command collection", () => {
		...
		it("suffixes duplicate extension commands in insertion order", async () => {
			const cmdCode = (description: string) => `
				export default function(bailu) {
					bailu.registerCommand("shared-cmd", {
	...
	describe("message and entry renderers", () => {
		it("gets Markdown transformers in extension load order", async () => {
			const extCode = `
				export default function(bailu) {
					bailu.registerMarkdownTransformer((markdown) => markdown);
				}
			`;
		...
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(bailu) {
					bailu.registerMessageRenderer("my-type", (message, options, theme) => null);
				}
			`;
		...
		it("gets entry renderer by type", async () => {
			const extCode = `
				export default function(bailu) {
					bailu.registerEntryRenderer("my-entry", (entry, options, theme) => null);
				}
			`;
	...
	describe("flags", () => {
		...
		it("keeps first flag when two extensions register the same name", async () => {
			const first = `
				export default function(bailu) {
					bailu.registerFlag("shared-flag", {
						description: "first",
						type: "boolean",
						default: true,
					});
	...
	describe("before_agent_start", () => {
		it("keeps ctx.getSystemPrompt() in sync with chained system prompt updates", async () => {
			const extCode1 = `
				export default function(bailu) {
					bailu.on("before_agent_start", async (_event, ctx) => {
						return {
	...
	describe("tool_result chaining", () => {
		it("chains content modifications across handlers", async () => {
			const extCode1 = `
				export default function(bailu) {
					bailu.on("tool_result", async (event) => {
						return {
	...
	describe("hasHandlers", () => {
		it("returns true when handlers exist for event type", async () => {
			const extCode = `
				export default function(bailu) {
					bailu.on("tool_call", async () => undefined);
				}
			`;
	...
	describe("before_provider_headers", () => {
		it("lets a handler mutate headers in place and preserves existing headers", async () => {
			const extCode = `
				export default function(bailu) {
					bailu.on("before_provider_headers", (event) => {
		...
		it("isolates a throwing handler and still applies the others", async () => {
			const throwing = `
				export default function(bailu) {
					bailu.on("before_provider_headers", () => {
