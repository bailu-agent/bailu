...
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel, type Model } from "@bailu/ai/compat";
...
async function createRuntimeHost(options: { withAuth: boolean; responseDelayMs: number; model?: Model<any> }): Promise<{
	...
	const tempDir = join(tmpdir(), `bailu-rpc-prompt-${Date.now()}-${Math.random().toString(36).slice(2)}`);
