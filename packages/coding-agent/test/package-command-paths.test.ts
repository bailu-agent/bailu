...
describe("package commands", () => {
	...
	let originalBailuPackageDir: string | undefined;
...
const bailuPath = path.join(binDir, process.platform === "win32" ? "bailu.cmd" : "bailu");
fs.writeFileSync(
	bailuPath,
...
if (process.platform !== "win32") fs.chmodSync(bailuPath, 0o755);
`,
		);
		...
		vi.stubEnv("BAILU_INSTALLER_API_BASE", "https://example.test/api/installer/releases");
		vi.stubEnv("BAILU_MANAGED_INSTALL_ROOT", managedRoot);
		process.env.BAILU_PACKAGE_DIR = selfPackageDir;
	...
	beforeEach(() => {
		allowNetwork();
		tempDir = join(tmpdir(), `bailu-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalBailuPackageDir = process.env.BAILU_PACKAGE_DIR;
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".bailu"), { recursive: true });
		writeFileSync(join(projectDir, ".bailu", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
		...
		try {
			await expect(
				main(["list"], {
					extensionFactories: [
						(bailu) => {
							bailu.on("project_trust", () => ({ trusted: "yes" }));
						},
					],
				}),
	...
	it("does not prompt or ask extensions for project trust during update", async () => {
		...
		writeFileSync(
			join(projectDir, ".bailu", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".bailu"), { recursive: true });
		writeFileSync(join(projectDir, ".bailu", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".bailu", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".bailu", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("bailu install <source> [-l]");
	...
	it("lists bailu as the self-update alias in update help", async () => {
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).toContain("[source|self|bailu]");
			expect(stdout).not.toContain("[source|self|pi]");
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
			logSpy.mockRestore();
	...
	it("cycles project package overrides in config local mode", async () => {
		...
		storage.withLock("global", () => JSON.stringify({ packages: ["npm:bailu-tools"] }));
		const settingsManager = SettingsManager.fromStorage(storage, { projectTrusted: true });
		const resolvedPaths = extensionPaths(join(tempDir, "pkg"), "npm:bailu-tools", "user", ["bar.ts"]);
		...
		expect(settingsManager.getProjectSettings().packages).toEqual([
			{ source: "npm:bailu-tools", autoload: false, extensions: ["-extensions/bar.ts"] },
		]);
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "bailu --help" or "bailu install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: bailu install <source> [-l]");
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		const previousSkipVersionCheck = process.env.BAILU_SKIP_VERSION_CHECK;
		process.env.BAILU_SKIP_VERSION_CHECK = "1";
		...
		try {
			...
			expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
				`bailu is already up to date (v${VERSION})`,
			);
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
			if (previousSkipVersionCheck === undefined) {
				...
				process.env.BAILU_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			}
		}
	});
	...
	it("retries a transient self-update version check", async () => {
		...
		try {
			...
			if (previousSkipVersionCheck === undefined) delete process.env.BAILU_SKIP_VERSION_CHECK;
			else process.env.BAILU_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	});

	it("updates installer-managed Bailu through a staged immutable release", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			`Updated bailu from ${VERSION} to ${targetVersion}`,
		);
		expect(errorSpy).not.toHaveBeenCalled();
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("rejects a concurrent managed update", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).not.toContain("Updated bailu from");
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Another managed Bailu update is already running.",
		);
		expect(process.exitCode).toBe(1);
	});
	...
	it("rejects forced managed reinstalls", async () => {
		...
		expect(existsSync(npmRecordPath)).toBe(false);
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Managed bailu installations do not support --force",
		);
		expect(process.exitCode).toBe(1);
	});
	...
	it("keeps npm self-updates non-managed when the managed environment is inherited", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@bailu", "coding-agent");
		...
		vi.stubEnv("BAILU_MANAGED_INSTALL_ROOT", inheritedManagedRoot);
...
`,
		...
		try {
			...
			expect(stdout).toContain(`Updated bailu from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated bailu from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		const activePackageName = PACKAGE_NAME === "@new-scope/bailu" ? "@newer-scope/bailu" : "@new-scope/bailu";
		vi.stubGlobal(
			"fetch",
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		const selfPackageDir = join(globalRoot, "node_modules", "@bailu", "coding-agent");
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain("Updated bailu");
			expect(stderr).toContain("exited with code 23");
			expect(stderr).toContain("If pnpm reports missing package versions");
			expect(stderr).toContain("Run `pnpm store prune` and retry `bailu update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:bailu-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "bailu-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:bailu-formatter?");
			expect(stdout).not.toContain("Updated bailu-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:bailu-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
