import type { NativeClipboard } from "@bailu/tui";
...
vi.mock("@bailu/tui", () => ({ getNativeClipboard: mocks.getNativeClipboard }));
	...
	test("WSL: tries PowerShell before a broken native X11 bridge", async () => {
		...
		mocks.command.mockImplementation(async (command, args, options) => {
			...
			if (command === "powershell.exe") {
				...
				expect(spawnOptions.env?.BAILU_WSL_CLIPBOARD_IMAGE_PATH).toBeUndefined();
