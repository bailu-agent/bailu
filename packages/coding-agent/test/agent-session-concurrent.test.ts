...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `bailu-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(bailu) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = bailu;
			},
			(bailu) => {
		...
		expect(bailu).toBeDefined();

		bailu!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
