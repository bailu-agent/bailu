...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bailu-ext-test-"));
	...
	const extensionCode = `
		export default function(bailu) {
			bailu.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("loads the coding-agent entrypoint without rewriting bailu-ai provider subpaths", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "coding-agent-import.ts"),
			`
				import { getAgentDir } from "@bailu/coding-agent";
				void getAgentDir;
				export default function(bailu) {
					bailu.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only bailu-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@bailu/ai/oauth";
				void oauth;
				export default function(bailu) {
					bailu.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json bailu field", async () => {
	...
	it("keeps package.json bailu extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with bailu field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without bailu field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(bailu) {
				bailu.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
				bailu.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				bailu.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(bailu) {
				bailu.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(bailu) {
				bailu.on("agent_start", async () => {});
				bailu.on("tool_call", async (event) => undefined);
				bailu.on("agent_end", async () => {});
			}
		`;
