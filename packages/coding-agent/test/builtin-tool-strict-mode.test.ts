...
describe("strict built-in tools", () => {
	...
	it.each([undefined, "0", "1"])("prefers strict sampling with BAILU_EXPERIMENTAL=%s", (experimental) => {
		vi.stubEnv("BAILU_EXPERIMENTAL", experimental);
	...
	it.each([{ activeTools: [] }, { activeTools: ["read"] }, { activeTools: [...strictToolNames] }])(
		...
		async ({ activeTools }) => {
			const cwd = mkdtempSync(join(tmpdir(), "bailu-non-strict-tools-"));
				...
				extensionFactories: [
					(bailu) => {
						bailu.on("session_start", () => {
							...
							for (const name of strictToolNames) {
								bailu.registerTool({ ...definitions[name], constrainedSampling: false });
							}
							bailu.setActiveTools(activeTools);
						});
					},
				],
			});
