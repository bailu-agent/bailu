...
import { Agent, type AgentEvent, type AgentTool } from "@bailu/agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@bailu/ai/compat";
...
describe("AgentSession retry", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `bailu-retry-test-${Date.now()}`);
