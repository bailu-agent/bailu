...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const extensionFile = join(cachedDir, "bailu-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "bailu-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ pi: { extensions: ["./bailu-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "bailu-extensions/session-breakdown.ts")).toBe("// fresh");
		});
	});
});
