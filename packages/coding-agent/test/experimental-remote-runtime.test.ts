...
import { type Context, createFacetHost, defineFacet, defineService } from "@bailu/chord";
import { BACKGROUND_CONTEXT } from "@bailu/chord/context";
import { Client, ServerError as ClientServerError } from "@bailu/client";
import { createUnixTransportFactory } from "@bailu/client/unix";
import { afterEach, beforeEach, describe, expect, test, vi } from "vitest";
import { ExampleFacetService } from "../examples/plugins/bailu-example-plugin/src/contract.ts";
...
beforeEach(async () => {
	agentDir = await mkdtemp(join("/tmp", "bailu-experimental-agent-"));
	directories.add(agentDir);
	await configureExperimentalWorkerModel(agentDir);
	vi.stubEnv("BAILU_CODING_AGENT_DIR", agentDir);
...
describe("experimental durable server composition", () => {
	test("uses BAILU_SERVER_DIR and BAILU_SERVER_ID", async () => {
		const directory = await mkdtemp(join("/tmp", "bailu-server-dir-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		vi.stubEnv("BAILU_SERVER_DIR", directory);
		vi.stubEnv("BAILU_SERVER_ID", serverId);
	...
	test("serializes concurrent cold activation and retires after both clients leave", async () => {
		const directory = await mkdtemp(join("/tmp", "bailu-auto-server-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		vi.stubEnv("BAILU_SERVER_DIR", directory);
		vi.stubEnv("BAILU_SERVER_ID", serverId);
	...
	test("passes client plugin packages to a cold server and restores them for its next generation", async () => {
		const directory = await mkdtemp(join("/tmp", "bailu-auto-plugin-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
	...
	test("retires a cold server after its only Session attachment disconnects", async () => {
		const directory = await mkdtemp(join("/tmp", "bailu-auto-session-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		vi.stubEnv("BAILU_SERVER_DIR", directory);
		vi.stubEnv("BAILU_SERVER_ID", serverId);
	...
	test("runs and discovers multiple logical servers from one directory", async () => {
		const directory = await mkdtemp(join("/tmp", "bailu-multi-server-"));
		directories.add(directory);
	...
	test("loads conventional Session facets from multiple configured plugin packages", async () => {
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					name: "@bailu/second-session-plugin",
					version: "1.0.0",
			...
			writeFile(
				...
				'import { defineFacet, defineService } from "@bailu/chord"; const Service = defineService("test.second-plugin"); export default defineFacet({ id: "second-session-plugin", setup(env) { env.provide(Service, { async read() { return "second"; } }); } });\n',
			),
		]);
		const runtime = await startServer({ ...sessionWorkerModel, directory });
		servers.add(runtime);
		...
		await activated.plugins.prepareSession(
			{
				sessionId: "demo-1",
				packagePaths: [
					fileURLToPath(new URL("../examples/plugins/bailu-example-plugin", import.meta.url)),
					secondPackagePath,
				],
			},
			BACKGROUND_CONTEXT,
	...
	test("retires an unclaimed idle worker after replacement demand expires", async () => {
		const directory = await mkdtemp(join("/tmp", "bailu-orphan-worker-"));
		directories.add(directory);
