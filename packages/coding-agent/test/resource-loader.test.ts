...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "bailu-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".bailu", "prompts");
			...
			const projectSkillDir = join(cwd, ".bailu", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".bailu", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".bailu", "themes"), { recursive: true });
		...
		it("should load symlinked user and project extensions once", async () => {
			...
			writeFileSync(
				...
				`export default function(bailu) {
	bailu.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".bailu"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".bailu", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".bailu", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".bailu", "extensions");
...
export default function(bailu) {
	...
	bailu.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(bailu) {
	bailu.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".bailu", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
	...
	bailu.registerCommand("project-only", {
	...
	bailu.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.bailu", async () => {
			const bailuDir = join(cwd, ".bailu");
			mkdirSync(bailuDir, { recursive: true });
			writeFileSync(join(bailuDir, "SYSTEM.md"), "You are a helpful assistant.");
		...
		it("should skip project resources that require trust when project is not trusted", async () => {
			const bailuDir = join(cwd, ".bailu");
			const extensionsDir = join(bailuDir, "extensions");
			const skillDir = join(bailuDir, "skills", "project-skill");
			const promptsDir = join(bailuDir, "prompts");
			const themesDir = join(bailuDir, "themes");
			mkdirSync(extensionsDir, { recursive: true });
			mkdirSync(skillDir, { recursive: true });
			mkdirSync(promptsDir, { recursive: true });
			mkdirSync(themesDir, { recursive: true });
			writeFileSync(join(bailuDir, "SYSTEM.md"), "Project system prompt.");
...
Project skill content`,
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const bailuDir = join(cwd, ".bailu");
			mkdirSync(bailuDir, { recursive: true });
			writeFileSync(join(bailuDir, "APPEND_SYSTEM.md"), "Additional instructions.");
	...
	describe("system prompt sources", () => {
		it("exposes discovered project SYSTEM.md as the system prompt source", async () => {
			const bailuDir = join(cwd, ".bailu");
			const systemPromptPath = join(bailuDir, "SYSTEM.md");
			mkdirSync(bailuDir, { recursive: true });
		...
		it("exposes discovered APPEND_SYSTEM.md as an append system prompt source", async () => {
			const bailuDir = join(cwd, ".bailu");
			const appendSystemPromptPath = join(bailuDir, "APPEND_SYSTEM.md");
			mkdirSync(bailuDir, { recursive: true });
