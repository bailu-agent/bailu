...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@bailu/ai/compat";
import { getApiProvider, getModels, getSupportedThinkingLevels } from "@bailu/ai/compat";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `bailu-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
