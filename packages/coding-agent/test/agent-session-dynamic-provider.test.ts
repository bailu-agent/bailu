...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `bailu-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(bailu) => {
				bailu.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(bailu) => {
				bailu.on("session_start", () => {
					bailu.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("registers native bailu-ai providers during extension loading", async () => {
		const session = await createSession([
			(bailu) => {
				bailu.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-top-level"));
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(bailu) => {
				bailu.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						bailu.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
	...
	it("registers native bailu-ai providers at command time", async () => {
		const session = await createSession([
			(bailu) => {
				bailu.registerCommand("use-native", {
					description: "Use native provider",
					handler: async () => {
						bailu.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-command"));
					},
				});
			},
		]);
