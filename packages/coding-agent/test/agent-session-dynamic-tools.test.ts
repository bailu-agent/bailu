...
describe("AgentSession dynamic tool registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `bailu-dynamic-tool-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(session.systemPrompt).toContain(
			"You can inspect BAILU_* environment variables for current model and session details.",
		);
		await bashTool.execute("bash-env", { command: "printf ok" });
		expect(sessionEnv).toMatchObject({
			BAILU_SESSION_ID: session.sessionId,
			BAILU_SESSION_FILE: session.sessionFile,
			BAILU_PROVIDER: model.provider,
			BAILU_MODEL: model.id,
			BAILU_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("BAILU_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("BAILU_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("BAILU_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("BAILU_MODEL");
		expect(optedOutEnv).not.toHaveProperty("BAILU_REASONING_LEVEL");
