...
import type { OAuthCredentials } from "@bailu/ai";
import { getModel, streamSimple } from "@bailu/ai/compat";
import { builtinProviders } from "@bailu/ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.bailu/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".bailu", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.bailu/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.bailu/agent/auth.json
 */
...
/** Path to the real bailu agent config directory */
export const BAILU_AGENT_DIR = join(homedir(), ".bailu", "agent");

/**
 * Get an AuthStorage instance backed by ~/.bailu/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `bailu-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
