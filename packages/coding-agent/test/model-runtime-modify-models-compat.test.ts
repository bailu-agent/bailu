...
describe("extension provider model lifecycle", () => {
	it("registers native bailu-ai providers with their auth implementation", async () => {
	...
	it("preserves native deferred methods through provider overlays", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "bailu-native-provider-deferred-"));
	...
	it("applies models.json overrides above native providers", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "bailu-native-provider-"));
		const modelsPath = join(tempDir, "models.json");
		writeFileSync(
			modelsPath,
			JSON.stringify({
