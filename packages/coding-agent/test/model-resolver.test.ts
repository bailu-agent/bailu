...
import type { Model } from "@bailu/ai";
import { getModel, streamSimple } from "@bailu/ai/compat";
import { getBuiltinModels, getBuiltinProviders } from "@bailu/ai/providers/all";
...
describe("default model selection", () => {
	...
	describe("persisted default model scoping", () => {
		...
		async function createSession(options: { scoped: boolean; persistedScope?: string[] }) {
			const tempDir = join(tmpdir(), `bailu-default-scope-${Date.now()}-${Math.random().toString(36).slice(2)}`);
