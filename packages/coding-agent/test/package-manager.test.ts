...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.BAILU_OFFLINE;
		delete process.env.BAILU_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.BAILU_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .bailu skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .bailu", async () => {
			const extDir = join(tempDir, ".bailu", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".bailu"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".bailu", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".bailu", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".bailu", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".bailu", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".bailu", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		...
		it("should use the project .bailu dir as baseDir for project .bailu skills", async () => {
			const projectBaseDir = join(tempDir, ".bailu");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".bailu", "agent");
		...
		it("should dedupe user skill entries when ~/.bailu/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .bailu auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".bailu\n");

			const skillDir = join(tempDir, ".bailu", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with bailu manifest", async () => {
		...
		it("should keep bailu manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.bailu\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".bailu", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the bailu-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "bailu-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .bailu settings base", () => {
			...
			const rel = relative(join(tempDir, ".bailu"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in bailu manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "bailu-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "bailu-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:bailu-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:bailu-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".bailu"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".bailu", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".bailu", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".bailu", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".bailu", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".bailu", "npm", "node_modules", "example");
