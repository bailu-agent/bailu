...
- Enabled strict-prefer JSON-schema sampling by default for built-in `read`, `bash`, `powershell`, `edit`, and `write` tools, without requiring `BAILU_EXPERIMENTAL`. Extensions can re-register tool definitions with `constrainedSampling: false`.
...
- Restored the `@bailu/coding-agent/client` compatibility entry point.
...
- Fixed Windows shell aborts crashing Bailu when `taskkill.exe` is unavailable on `PATH` ([#6596](https://github.com/earendil-works/pi/issues/6596)).
...
- Fixed large tool results crossing the auto-compaction threshold being sent to the provider before compaction. Bailu now compacts between tool execution and the next assistant response in the same run, and restores interactive progress when that run resumes ([#6879](https://github.com/earendil-works/pi/issues/6879)).
...
- Changed experimental installer-managed installations so `bailu update` stages, verifies, and atomically activates the selected release in place. See [Install and Manage](docs/packages.md#install-and-manage).
...
- Changed inherited Anthropic, Azure OpenAI, Google, Mistral, and OpenAI adapters to send Bailu's default `User-Agent` unless overridden ([#8305](https://github.com/earendil-works/pi/issues/8305)).
...
- Fixed npm package update checks treating older registry versions as available updates, preventing `bailu update` from downgrading already-newer installed packages ([#8226](https://github.com/earendil-works/pi/issues/8226)).
- Fixed built-in llama.cpp models disappearing from `/model` when `/llama` refreshed a configured server under `BAILU_OFFLINE`, and included idle-slept `sleeping` router models plus autoloadable unloaded presets in the selectable catalog ([#8558](https://github.com/earendil-works/pi/pull/8558) by [@cristinaponcela](https://github.com/cristinaponcela)).
...
- Added experimental strict JSON-schema constrained sampling for the default `read`, `bash`, `edit`, and `write` tools under `BAILU_EXPERIMENTAL=1`.
...
- Changed inherited Kimi Coding requests to use bailu's runtime `User-Agent` header.
...
- Documented the generic `AI_AGENT=bailu` process marker and how it differs from `BAILU_CODING_AGENT=true` ([#7747](https://github.com/earendil-works/pi/issues/7747)).
...
- Fixed split `Alt+Enter` input over SSH being misread as Escape, added `BAILU_TUI_ESC_TIMEOUT` for high-latency terminals, and limited that timeout to lone Escape input ([#7899](https://github.com/earendil-works/pi/pull/7899) by [@powerfooI](https://github.com/powerfooI)).
...
- **Authentication readiness checks** — Use `bailu auth check` to verify provider or model credentials, optionally emitting the resolved credential.
...
- Added `bailu auth check` provider/model auth preflight with optional credential output ([#7152](https://github.com/earendil-works/pi/issues/7152)).
...
- Softened the bash tool's `BAILU_*` environment guideline in an attempt to reduce unnecessary inspection commands ([#7128](https://github.com/earendil-works/pi/issues/7128)).
...
- Renamed the inherited bailu-ai `ModelsStreamTransforms` interface to `ModelsRequestTransforms` because its header transformation now applies to all authenticated provider requests.
...
- Replaced dynamic provider refresh context store access with the read-only `context.stored` snapshot and generation-checked `context.publish()` transaction.
  ...
  **Handwritten native `Provider.refreshModels()`:** replace direct store access and pre-publication mutation with generation-guarded publications.
  ...
  For the config-form `pi.registerProvider(name, { refreshModels })`, callbacks that only return models remain unchanged; bailu publishes the returned list. If such a callback previously used `context.store` for custom persistence, read `context.stored` and call `context.publish({ persist: entry })`. In `publish()`, omit `persist` to leave storage unchanged, pass a `ModelsStoreEntry` to write it, or pass `persist: null` to delete it.

- Replaced the inherited bailu-agent-core harness session model with the v4 lane-based `Session`, `SessionStorage`, and `SessionRepo` APIs, including durable operation records, global facts, shared sequence numbers, and tree-scoped lane views.
- Promoted the inherited v2 session and `AgentHarness` API from bailu-agent-core's experimental entrypoint to its default export and removed the experimental subpaths.
- Removed the inherited legacy JSONL and in-memory repository APIs. Use bailu-agent-core's v4 `JsonlSessionRepo` or `InMemorySessionRepo`, both implementing the new `SessionRepo` contract.
- Added the inherited required bailu-agent-core `FileSystem.renameFile()` operation for atomic JSONL publication; custom harness file-system implementations must provide same-filesystem replacement semantics ([#7707](https://github.com/earendil-works/pi/pull/7707) by [@davidbrai](https://github.com/davidbrai)).
...
- Added experimental remote-session client APIs: the transport-neutral `BailuClient`, CBOR protocol, Unix-socket transport, and `@bailu/coding-agent/client` `RemoteSession` controller with transcript reducers. See [Bailu Client](../client/README.md) and [Remote Protocol](../protocol/README.md) ([#7344](https://github.com/earendil-works/pi/pull/7344), [#7348](https://github.com/earendil-works/pi/pull/7348), [#7371](https://github.com/earendil-works/pi/pull/7371), [#7409](https://github.com/earendil-works/pi/pull/7409)).
...
- Added `AI_AGENT=bailu` to CLI and RPC child-process environments for generic agent attribution. See [Environment Variables](README.md#environment-variables) ([#7493](https://github.com/earendil-works/pi/pull/7493) by [@renaudhartert-db](https://github.com/renaudhartert-db)).
...
- Added inherited optional cancellation to bailu-ai `ModelsStore` reads, writes, and deletions; catalog orchestration binds these waits to the provider refresh signal.
...
- Fixed the inherited bailu-tui npm package omitting the source and build scripts needed to rebuild its Windows and Darwin native addons.
...
- **Credential export for external clients** — `bailu auth print-api-key` and `bailu auth print-bearer-token` export configured credentials with automatic OAuth refresh and minimum-validity enforcement.
...
- Added `bailu auth print-api-key` and `bailu auth print-bearer-token` commands for exporting configured credentials to external clients, including automatic OAuth refresh and configurable minimum token validity ([#7168](https://github.com/earendil-works/pi/pull/7168)).
...
- Exposed `BAILU_SESSION_ID`, `BAILU_SESSION_FILE`, `BAILU_PROVIDER`, `BAILU_MODEL`, and `BAILU_REASONING_LEVEL` to commands run by built-in and factory-created bash tools. See [Bash Tool Session Environment](docs/environment-variables.md#bash-tool-session-environment).
...
- Fixed TUI debug and crash logs to respect custom agent directories instead of always writing under `~/.bailu/agent` ([#6958](https://github.com/earendil-works/pi/pull/6958) by [@davidbrai](https://github.com/davidbrai)).
...
- Fixed explicit self-updates when `BAILU_SKIP_VERSION_CHECK` is set ([#6977](https://github.com/earendil-works/pi/issues/6977)).
...
- **Full provider extensions** — Extensions can register complete bailu-ai providers with authentication, model refresh, filtering, and custom streaming. See [Register New Provider](docs/custom-provider.md#register-new-provider).
...
- Added extension registration for complete bailu-ai providers, including native authentication, model refresh, filtering, and streaming behavior.
...
- Fixed inherited `bailu-ai` package metadata to avoid repeated consumer lockfile changes ([#6812](https://github.com/earendil-works/pi/pull/6812) by [@jmfederico](https://github.com/jmfederico)).
...
- **Live model catalog refresh** — `/model` refreshes configured providers in the background, and `bailu update --models` forces an immediate refresh. See [Install and Manage](docs/packages.md#install-and-manage).
...
- Replaced the SDK's `CreateAgentSessionOptions.authStorage` and `modelRegistry` options with the async `modelRuntime` option. `AuthStorage` and its storage backends are no longer exported; use `ModelRuntime` (or a custom bailu-ai `CredentialStore`), or `readStoredCredential()` for one-off reads of auth.json.
- Removed redundant `ModelRuntime.getAll()`, `find()`, `getSnapshot()`, and `getAuthOptions()` projections. Use the bailu-ai `Models` methods `getModels()`, `getModel()`, `getProviders()`, and `checkAuth()` directly.
...
- Moved canonical dynamic catalog refresh to async `ModelRuntime.refresh()`/bailu-ai `Models.refresh()`. Legacy extension OAuth `modifyModels` remains supported as a synchronous compatibility projection after credential initialization.

### Added

- Added `ModelRuntime` as the canonical async SDK and internal model/auth facade while preserving the synchronous extension-facing `ModelRegistry` API. `ModelRuntime.create()` accepts any bailu-ai `CredentialStore` through its `credentials` option.
- Added provider-owned `/login` discovery directly from registered bailu-ai providers, including ambient auth status and informational links.
...
- Added `bailu update --models` to force an immediate model catalog refresh without updating bailu or extensions.
...
- Changed `ModelRuntime` to compose built-in providers, immutable `models.json` configuration, and extension overlays through ad-hoc bailu-ai provider methods.
...
- Fixed configured-provider catalog refresh to parse pi.dev's model-ID keyed responses, throttle checks to once per four hours, send the versioned bailu user agent, treat unimplemented routes as unavailable overlays, and show concise refresh status in `/model`.
...
- **Project-local resource configuration** - `bailu config -l` and Tab switching manage global vs project-local package resources. See [Enable and Disable Resources](docs/packages.md#enable-and-disable-resources).
...
- Added project-local resource override management to `bailu config`, including project mode startup with `bailu config -l` and Tab switching between global and project scopes ([#6309](https://github.com/earendil-works/pi/pull/6309)).
...
- Fixed inherited OpenAI Codex user-agent construction to synchronously load Node OS metadata, avoiding a startup race that could report `bailu (browser)` in Node/Bun.
- Fixed `bailu update` for pnpm installations to suggest pruning pnpm's self-update cache when the executable points at a removed cached version ([#6279](https://github.com/earendil-works/pi/pull/6279) by [@rajp152k](https://github.com/rajp152k)).
...
- Fixed `--session-id` startup to warn when no existing project session has that id and bailu creates a new session ([#6407](https://github.com/earendil-works/pi/issues/6407)).
...
- Added a package `./rpc-entry` export for launching Bailu directly in RPC mode.
...
- Changed inherited bailu-ai `ApiKeyCredential` to use the `auth.json`-compatible discriminator `type: "api_key"` and provider-scoped `env` values instead of `type: "api-key"` and metadata.
...
- Restored inherited temporary legacy per-API stream aliases such as `streamSimpleOpenAICompletions` on the bailu-ai compat entrypoint ([#6016](https://github.com/earendil-works/pi/issues/6016), [#6017](https://github.com/earendil-works/pi/issues/6017)).
...
- bailu-ai's old global API (`stream`/`complete`/`completeSimple`, `getModel`/`getModels`/`getProviders`, `registerApiProvider`, `getEnvApiKey`, ...) moved off the `@bailu/ai` root entrypoint to `@bailu/ai/compat`. Extensions are not affected at runtime: the extension loader resolves the bailu-ai root to the compat entrypoint (a strict superset), so existing extensions keep working unchanged. Extension sources that typecheck against bailu-ai's published types should switch those imports to `@bailu/ai/compat` (or migrate to the new `createModels()`/provider-factory API). The compat entrypoint and the loader alias will be removed in a future release with a migration guide.
...
- Fixed extension-related crash and startup-failure reporting to suggest restarting with `bailu -ne`.
...
- Fixed `bailu --resume` to load user package themes and resolve automatic light/dark theme settings.
...
- Removed inherited selective-provider `@bailu/ai/base` and `@bailu/agent-core/base` entrypoints; use the root packages with explicit `Models` provider factories instead.
...
- **Safer update flow** - `bailu update` installs the exact checked Bailu version, and update notices show the changelog URL, making upgrades more predictable. See [Install and Manage](docs/packages.md#install-and-manage).
...
- Fixed `bailu update` to install the exact version returned by the Bailu update check, make `--force` reinstall that checked version, fail instead of falling back to an unversioned reinstall when no version is available, and report both the old and updated versions.
...
- **Selective provider base entry points** - SDK users can pair `@bailu/ai/base` and `@bailu/agent-core/base` with explicit provider registration to keep bundled applications from including unused provider transports. See [`bailu-ai` Base Entry Point](../ai/README.md#base-entry-point) and [`bailu-agent-core` Base Entry Point](../agent/README.md#base-entry-point).
...
- Added inherited `@bailu/ai/base` and `@bailu/agent-core/base` entry points for selective provider registration in bundled applications ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
- Added inherited Mistral prompt caching using the bailu session ID as `prompt_cache_key`, including cached-token usage and cost accounting ([#5854](https://github.com/earendil-works/pi/issues/5854)).
...
- **Self-only updates by default** - `bailu update` now updates bailu only, with `bailu update --all` for updating bailu and packages together. See [Install and Manage](docs/packages.md#install-and-manage).
...
- Exported `CONFIG_DIR_NAME` from the coding-agent public API so extensions can resolve project config paths without hardcoding `.bailu` ([#5869](https://github.com/earendil-works/pi/pull/5869) by [@xl0](https://github.com/xl0)).
...
- Changed bare `bailu update` to update only bailu, added `bailu update --all` for updating bailu and extensions together, and clarified extension update prompts.
- Reserved `/` in theme names for automatic light/dark theme settings.
- Updated extension docs, examples, runtime help, trust prompts, and config labels to use the configured project config directory instead of hardcoded `.bailu` paths.
...
- **Global HTTP proxy setting** - Configure `httpProxy` once in global settings to apply `HTTP_PROXY` and `HTTPS_PROXY` to Bailu-managed HTTP clients. See [Network](docs/settings.md#network).
- **Vercel AI Gateway attribution** - Vercel AI Gateway requests now include Bailu attribution headers by default. See [API Keys](docs/providers.md#api-keys).
...
- Added a global `httpProxy` setting that applies as `HTTP_PROXY` and `HTTPS_PROXY` for Bailu-managed HTTP clients ([#5790](https://github.com/earendil-works/pi/issues/5790)).
- Added `auth.json` API key `env` values so provider-specific environment overrides can be scoped to Bailu and propagated to inherited provider configuration ([#5728](https://github.com/earendil-works/pi/issues/5728)).
...
- Fixed successful `bailu update` on Windows to exit naturally instead of calling `process.exit(0)`, avoiding a Node.js/libuv assertion after version-check network requests ([#5805](https://github.com/earendil-works/pi/issues/5805)).
...
- **Automatic first-run theme selection** - bailu detects the terminal background on first run and defaults to the `dark` or `light` theme. See [Selecting a Theme](docs/themes.md#selecting-a-theme).
...
- Fixed package commands such as `bailu list`, `bailu install`, and `bailu update` to terminate after completing even if an extension leaves background handles open ([#5687](https://github.com/earendil-works/pi/issues/5687)).
- Fixed `bailu update` for pnpm global installs whose configured `global-bin-dir` no longer matches the active pnpm home ([#5689](https://github.com/earendil-works/pi/issues/5689)).
...
- Added an experimental first-time setup flow behind `BAILU_EXPERIMENTAL=1` that asks for a dark/light theme choice (preselecting the detected appearance) and opt-in analytics data sharing on first launch with the default agent directory; opting in stores a `trackingId` in `settings.json` ([#5587](https://github.com/earendil-works/pi/pull/5587) by [@vegarsti](https://github.com/vegarsti)).
...
- Fixed project trust detection to ignore global `~/.bailu/agent` state when running from `$HOME`, and made `bailu update` use only saved or explicit project trust without prompting ([#5619](https://github.com/earendil-works/pi/issues/5619)).
...
- Added Claude Fable 5 model support inherited from `@bailu/ai` for the Anthropic and Amazon Bedrock providers, with adaptive thinking and `xhigh` effort support.
...
- **Project trust for local inputs** - Bailu now asks before loading project-local settings, resources, instructions, and packages, with saved decisions and `--approve` / `--no-approve` controls for non-interactive modes. See [Project Trust](README.md#project-trust).
...
- Fixed `/reload` to persist project trust when an implicitly trusted session creates a project `.bailu` directory.
...
- Added MiniMax-M3 model support inherited from `@bailu/ai` for the `minimax` and `minimax-cn` direct providers ([#5313](https://github.com/earendil-works/pi/issues/5313)).
...
- Fixed temporary extension package installs to use a private `~/.bailu/agent/tmp/extensions` directory with `0700` permissions instead of `os.tmpdir()/bailu-extensions`.
...
- Added custom Amazon Bedrock request header support inherited from `@bailu/ai` ([#5178](https://github.com/earendil-works/pi-mono/pull/5178) by [@stephanmck](https://github.com/stephanmck)).
...
- Fixed self-update commands to bypass npm, pnpm, and Bun minimum release age gates for explicit `bailu update` runs ([#4929](https://github.com/earendil-works/pi/issues/4929)).
...
- Fixed managed npm extension updates to avoid package managers installing or resolving bailu host packages as peer dependencies ([#4907](https://github.com/earendil-works/pi/issues/4907)).
...
- Fixed provider retry controls so `retry.provider.maxRetries` is honored, SDK retries default to `0`, and quota/billing 429s are not retried behind Bailu's retry handling ([#4991](https://github.com/earendil-works/pi-mono/pull/4991) by [@mitsuhiko](https://github.com/mitsuhiko)).
...
- **More reliable package updates** - `bailu update` and git package installs now reconcile pinned git refs and keep package settings intact. See [Packages](docs/packages.md).
...
- Fixed `bailu update` to reconcile git-pinned packages to their configured ref ([#4869](https://github.com/earendil-works/pi/issues/4869)).
...
- Fixed Amazon Bedrock provider loading under strict package managers by inheriting the declared `@smithy/node-http-handler` dependency from `@bailu/ai` ([#4842](https://github.com/earendil-works/pi/issues/4842)).
...
- Published a 0.74.2 rescue release that tells Node 20 users to upgrade Node before updating to newer Bailu versions ([#4876](https://github.com/earendil-works/pi/issues/4876)).
...
- Fixed the system prompt to tell models to resolve bailu docs and examples under the absolute package paths before reading topic-specific relative references ([#4752](https://github.com/earendil-works/pi/issues/4752)).
...
- Fixed loader indicator startup inherited from `@bailu/tui` so initialization cannot run before frames are available.
- Fixed OpenAI-compatible default output token requests inherited from `@bailu/ai` to avoid reserving impossible context windows on servers such as vLLM ([#4675](https://github.com/earendil-works/pi/issues/4675)).
- Fixed OpenAI prompt cache keys inherited from `@bailu/ai` to stay within the 64-character provider limit ([#4720](https://github.com/earendil-works/pi/issues/4720)).
...
- Fixed Xiaomi MiMo generated model metadata to replay assistant tool-call messages with `reasoning_content` for thinking-mode multi-turn requests, inherited from `@bailu/ai` ([#4678](https://github.com/earendil-works/pi/issues/4678)).
...
- Fixed Windows npm self-updates to move loaded native dependency packages out of the active install before reinstalling bailu ([#4157](https://github.com/earendil-works/pi/issues/4157)).
- Fixed `bailu update --self` detection for pnpm v11 global installs whose package path resolves through the pnpm store ([#4647](https://github.com/earendil-works/pi/issues/4647)).
...
- Fixed Anthropic-compatible API-key requests to ignore unrelated `ANTHROPIC_AUTH_TOKEN` environment values, avoiding invalid bearer credentials for providers such as Xiaomi MiMo inherited from `@bailu/ai` ([#4342](https://github.com/earendil-works/pi/issues/4342)).
- Fixed Amazon Bedrock message conversion to skip unknown content blocks instead of failing the stream, inherited from `@bailu/ai` ([#4223](https://github.com/earendil-works/pi/issues/4223)).
- Fixed Azure OpenAI Responses and OpenAI Responses error formatting to prefix HTTP status codes onto `errorMessage`, so transient 5xx and 429 errors are correctly matched by the agent-level auto-retry classifier inherited from `@bailu/ai` ([#4232](https://github.com/earendil-works/pi/issues/4232)).
- Fixed OpenCode Go Kimi reasoning replay by normalizing streamed `reasoning` fields back to `reasoning_content` for OpenCode Go only, inherited from `@bailu/ai` ([#4251](https://github.com/earendil-works/pi/issues/4251)).
- Fixed Xiaomi MiMo model metadata to use the OpenAI-compatible endpoints and `openai-completions` API, restoring multi-turn thinking/tool-call sessions inherited from `@bailu/ai` ([#4505](https://github.com/earendil-works/pi/issues/4505)).
- Fixed JSON parse failures for compressed fetch responses under Node 26.0 by installing undici fetch globals alongside bailu's global dispatcher ([#4650](https://github.com/earendil-works/pi/issues/4650), [#4652](https://github.com/earendil-works/pi/issues/4652), [#4653](https://github.com/earendil-works/pi/issues/4653)).
...
- Removed non-working OpenAI Codex fast model variants inherited from `@bailu/ai`.
...
- Fixed OpenAI Codex generated model metadata to use the current upstream model list inherited from `@bailu/ai` ([#4603](https://github.com/earendil-works/pi-mono/pull/4603) by [@mattiacerutti](https://github.com/mattiacerutti)).
- Fixed GitHub Copilot GPT model thinking metadata inherited from `@bailu/ai` to map unsupported minimal thinking to low ([#4622](https://github.com/earendil-works/pi-mono/pull/4622) by [@mattiacerutti](https://github.com/mattiacerutti)).
- Fixed user-scoped npm bailu packages to install under `~/.bailu/agent/npm/` instead of npm's global package root, avoiding permission errors with system-managed Node installs ([#4587](https://github.com/earendil-works/pi/issues/4587)).
...
- Fixed default output token requests for models whose advertised output limit is effectively their full context window, avoiding impossible provider requests inherited from `@bailu/ai` ([#4614](https://github.com/earendil-works/pi/issues/4614)).

## [0.74.1] - 2026-05-16

### New Features

- **Image generation support** - Added image generation APIs, generated image model metadata, and built-in OpenRouter image generation support inherited from `@bailu/ai`.
...
- **Improved terminal and markdown rendering** - Added markdown list indentation, task-list checkbox rendering, large markdown robustness, and inline image placement fixes inherited from `@bailu/tui`.

### Added

- Added image generation support from `@bailu/ai`, including image generation APIs, image model metadata, and built-in OpenRouter image generation support ([#3887](https://github.com/earendil-works/pi-mono/pull/3887) by [@cristinaponcela](https://github.com/cristinaponcela)).
...
- Fixed Node 26 OpenAI-compatible streams timing out after five idle minutes by routing global fetch through bailu's undici dispatcher ([#4519](https://github.com/earendil-works/pi/issues/4519)).
...
- Fixed OpenRouter cached-token usage accounting, Fireworks caching compatibility, and OpenAI Codex WebSocket proxy handling inherited from `@bailu/ai`.
- Fixed markdown list wrapping, task-list checkboxes, large markdown rendering, WezTerm Kitty keyboard escape handling, and short-viewport inline image placement inherited from `@bailu/tui`.
...
- **Self-update support for the npm scope migration**: `bailu update --self` now supports the upcoming package rename from `@mariozechner/bailu-coding-agent` to `@bailu/coding-agent`. After the new package is published, existing global installs can update through the normal self-update flow; bailu will uninstall the old global package and install the package name returned by the version check endpoint.
...
- Changed `bailu update --self` to honor the active package name returned by the Bailu version check endpoint, defaulting to the current package when omitted and uninstalling the old global package before installing a renamed package.
...
- Fixed `bailu -p` treating prompts that start with YAML frontmatter as extension flags instead of user messages ([#4163](https://github.com/badlogic/pi-mono/issues/4163)).
...
- **Compact read rendering** - Interactive `read` output for Bailu docs, context files, and skills is collapsed by default and shows selected line ranges.
...
- Changed `read` tool rendering to collapse Bailu documentation, AGENTS/CLAUDE context files, and `SKILL.md` contents by default in interactive output.
...
- **Self-update detection fix** - `bailu` now correctly identifies and applies available updates. ([#3942](https://github.com/badlogic/pi-mono/issues/3942), [#3980](https://github.com/badlogic/pi-mono/issues/3980), [#3922](https://github.com/badlogic/pi-mono/issues/3922)).

### Breaking Changes

- Replaced `compat.reasoningEffortMap` in `models.json` and `pi.registerProvider()` model definitions with model-level `thinkingLevelMap` ([#3208](https://github.com/badlogic/pi-mono/issues/3208)). Migration: move old mappings from `compat.reasoningEffortMap` to `thinkingLevelMap`. Use string values for provider-specific thinking values and `null` for unsupported bailu levels that should be hidden and skipped by cycling. See `docs/models.md#thinking-level-map` and `docs/custom-provider.md`.
...
- Added `shouldStopAfterTurn` agent loop callback for post-turn stop control, inherited from `@mariozechner/bailu-agent-core`. See [`packages/agent/README.md`](https://github.com/badlogic/pi-mono/blob/main/packages/agent/README.md).
...
- Fixed self-update detection so `bailu` correctly identifies when a newer version is available and applies updates ([#3942](https://github.com/badlogic/pi-mono/issues/3942), [#3980](https://github.com/badlogic/pi-mono/issues/3980), [#3922](https://github.com/badlogic/pi-mono/issues/3922)).
...
- `BAILU_CODING_AGENT_SESSION_DIR` configures session storage from the environment. See [docs/usage.md#environment-variables](docs/usage.md#environment-variables).
...
- Added Mistral Medium 3.5 built-in model support via `@mariozechner/bailu-ai` ([#4009](https://github.com/badlogic/pi-mono/pull/4009) by [@technocidal](https://github.com/technocidal)).
...
- Added `BAILU_CODING_AGENT_SESSION_DIR` as an environment equivalent to `--session-dir` ([#4027](https://github.com/badlogic/pi-mono/issues/4027)).
...
- Fixed the coding-agent README See Also link to point at `@mariozechner/bailu-agent-core` ([#4023](https://github.com/badlogic/pi-mono/issues/4023)).
...
- Bailu update checks now use `pi.dev` and identify Bailu with a `bailu/<version>` user agent. See [docs/packages.md](docs/packages.md). ([#3877](https://github.com/badlogic/pi-mono/pull/3877) by [@mitsuhiko](https://github.com/mitsuhiko))
...
- Changed Bailu version checks to identify Bailu with a `bailu/<version>` user agent ([#3877](https://github.com/badlogic/pi-mono/pull/3877) by [@mitsuhiko](https://github.com/mitsuhiko)).
...
- Fixed `bailu update` to skip self-update reinstalls when the installed version is already current ([#3853](https://github.com/badlogic/pi-mono/issues/3853)).
- Fixed Cloudflare Workers AI attribution headers to honor the install telemetry setting.
- Fixed `bailu update --self` detection and execution for Windows package-manager shim installs, including symlinked global package roots, and print the manual fallback command when self-update fails ([#3857](https://github.com/badlogic/pi-mono/issues/3857)).
...
- Fixed packaged `bailu` startup failing because the session selector imported a source-only utility path.

## [0.70.3] - 2026-04-27

### New Features

- `bailu update` can now update bailu itself in addition to installed bailu packages. See [docs/packages.md](docs/packages.md). ([#3680](https://github.com/badlogic/pi-mono/pull/3680) by [@mitsuhiko](https://github.com/mitsuhiko))
...
- Added `bailu update` support for updating bailu itself in addition to installed bailu packages ([#3680](https://github.com/badlogic/pi-mono/pull/3680) by [@mitsuhiko](https://github.com/mitsuhiko)).
...
- Fixed Windows git package installs to bypass `cmd.exe` for native git commands, so install paths containing spaces no longer break `bailu install git:...` with `fatal: Too many arguments` ([#3642](https://github.com/badlogic/pi-mono/issues/3642))
...
- Improved stale extension context errors after session replacement or reload to tell extension authors to avoid captured `bailu`/command `ctx` and use `withSession` for post-replacement work.
...
- Fixed remaining hardcoded `bailu` / `.bailu` branding to route through `APP_NAME` and `CONFIG_DIR_NAME` extension points, so SDK rebrands get consistent naming in `/quit` description, `process.title`, and the project-local extensions directory ([#3583](https://github.com/badlogic/pi-mono/pull/3583) by [@jlaneve](https://github.com/jlaneve))
- Fixed `bailu-coding-agent` shipping `uuid@11`, which triggered `npm audit` moderate vulnerability reports for downstream installs; the package now depends on `uuid@14` ([#3577](https://github.com/badlogic/pi-mono/issues/3577))
...
- Added `BAILU_OAUTH_CALLBACK_HOST` support for built-in OAuth login flows, allowing local callback servers used by `bailu auth` to bind to a custom interface instead of hardcoded `127.0.0.1` ([#3409](https://github.com/badlogic/pi-mono/pull/3409) by [@Michaelliv](https://github.com/Michaelliv))
...
- Changed `bailu update` to batch npm package updates per scope and run git package updates with bounded parallelism, reducing multi-package update time while preserving skip behavior for pinned and already-current packages ([#2980](https://github.com/badlogic/pi-mono/issues/2980))
...
- Fixed `bailu update` reinstalling npm packages that are already at the latest published version by checking the installed package version before running `npm install <pkg>@latest` ([#3000](https://github.com/badlogic/pi-mono/issues/3000))
...
- Fixed Windows pnpm global install detection to recognize `\\.pnpm\\` store paths, so update notices now suggest `pnpm install -g @mariozechner/bailu-coding-agent` instead of falling back to npm ([#3378](https://github.com/badlogic/pi-mono/issues/3378))
- Fixed missing `@sinclair/typebox` runtime dependency in `@mariozechner/bailu-coding-agent`, so strict pnpm installs no longer fail with `ERR_MODULE_NOT_FOUND` when starting `bailu` ([#3434](https://github.com/badlogic/pi-mono/issues/3434))
- Fixed xterm uppercase typing in the interactive editor by decoding printable `modifyOtherKeys` input and normalizing shifted letter matching, so `Shift+letter` no longer disappears in `bailu` ([#3436](https://github.com/badlogic/pi-mono/issues/3436))
...
- Fixed skill resolution to dedupe symlinked aliases by canonical path, so `bailu config` no longer shows duplicate skill entries when `~/.bailu/agent/skills` points to `~/.agents/skills` ([#3417](https://github.com/badlogic/pi-mono/pull/3417) by [@rwachtler](https://github.com/rwachtler))
- Fixed OpenRouter request attribution to include Bailu app headers (`HTTP-Referer: https://pi.dev`, `X-OpenRouter-Title: bailu`, `X-OpenRouter-Categories: cli-agent`) when sessions are created through the coding-agent SDK and install telemetry is enabled ([#3414](https://github.com/badlogic/pi-mono/issues/3414))
...
- Fixed missing root exports for `RpcClient` and RPC protocol types from `@mariozechner/bailu-coding-agent`, so ESM consumers can import them from the main package entrypoint ([#3275](https://github.com/badlogic/pi-mono/issues/3275))
...
- Fixed Bun binary asset path resolution to honor `BAILU_PACKAGE_DIR` for built-in themes, HTML export templates, and interactive bundled assets ([#3074](https://github.com/badlogic/pi-mono/issues/3074))
...
Why this exists:
- Bailu needs a reliable per-version usage signal to understand whether releases are being adopted and to help justify funding continued development.
- npm download counts are not a reliable proxy for actual Bailu usage.

How it works:
- It only runs in interactive mode.
- It does not run in RPC mode, print mode, JSON mode, or SDK mode.
- On a fresh interactive install, Bailu writes `lastChangelogVersion`, then sends the ping.
- On later interactive startups, if the local changelog contains entries newer than the previously stored `lastChangelogVersion`, Bailu writes the new `lastChangelogVersion`, then sends the ping.
- The request is fire-and-forget. Startup does not wait for it, and any errors are ignored.

What data is collected:
- Only the Bailu version in the request path, for example `https://pi.dev/install?version=0.67.1`.
...
- `BAILU_CODING_AGENT=true` environment variable set at startup so subprocesses can detect they are running inside the coding agent.
...
- Set `BAILU_CODING_AGENT=true` environment variable at startup so sub-processes can detect they are running inside the coding agent ([#2868](https://github.com/badlogic/pi-mono/issues/2868))
...
- Fixed Gemma 4 thinking level mapping to route between `MINIMAL` and `HIGH`, and map Bailu reasoning levels to the model's supported thinking levels ([#2903](https://github.com/badlogic/pi-mono/pull/2903) by [@aadishv](https://github.com/aadishv))
...
- Fixed OpenAI-compatible completions streaming usage accounting to preserve `prompt_tokens_details.cache_write_tokens` and normalize OpenRouter `cached_tokens`, preventing incorrect cache read/write token and cost reporting in bailu ([#2802](https://github.com/badlogic/pi-mono/issues/2802))
...
import {
  ...
  getAgentDir,
  SessionManager,
} from "@mariozechner/bailu-coding-agent";
...
- Fixed monorepo `tsconfig.json` path mappings to resolve `@mariozechner/bailu-ai` subpath exports to source files in development checkouts ([#2625](https://github.com/badlogic/pi-mono/pull/2625) by [@ferologics](https://github.com/ferologics))
...
- Fixed skill discovery to stop recursing once a directory contains `SKILL.md`, and to ignore root `*.md` files in `.agents/skills` while keeping root markdown skill files supported in `~/.bailu/agent/skills`, `.bailu/skills`, and package `skills/` directories ([#2603](https://github.com/badlogic/pi-mono/issues/2603))
...
- Added a startup onboarding hint in the interactive header telling users bailu can explain its own features and documentation ([#2620](https://github.com/badlogic/pi-mono/pull/2620) by [@ferologics](https://github.com/ferologics))
...
- Added support for `BAILU_TUI_WRITE_LOG` directory paths, creating a unique log file (`tui-<timestamp>-<pid>.log`) per instance for easier debugging of multiple bailu sessions ([#2508](https://github.com/badlogic/pi-mono/pull/2508) by [@mrexodia](https://github.com/mrexodia))
...
- Fixed `models.json` shell-command auth and headers to resolve at request time instead of being cached into long-lived model state. bailu now leaves TTL, caching, and recovery policy to user-provided wrapper commands because arbitrary shell commands need provider-specific strategies ([#1835](https://github.com/badlogic/pi-mono/issues/1835))
...
- Added missing `ajv` direct dependency; previously relied on transitive install via `@mariozechner/bailu-ai` which broke standalone installs ([#2252](https://github.com/badlogic/pi-mono/issues/2252))
...
- Fixed `bailu update` for git packages to fetch only the tracked target branch with `--no-tags`, reducing unrelated branch and tag noise while preserving force-push-safe updates ([#2548](https://github.com/badlogic/pi-mono/issues/2548))
...
- Built-in tools now work like custom tools in extensions. To get built-in tool definitions, import `readToolDefinition` / `createReadToolDefinition()` and the equivalent `bash`, `edit`, `write`, `grep`, `find`, and `ls` exports from `@mariozechner/bailu-coding-agent`.
...
- Fixed `bailu update` for git packages to skip destructive reset, clean, and reinstall steps when the fetched target already matches the local checkout ([#2503](https://github.com/badlogic/pi-mono/issues/2503))
...
- Added `ToolCallEventResult` to the `@mariozechner/bailu-coding-agent` top-level and core extension exports so extension authors can type explicit `tool_call` handler return values ([#2458](https://github.com/badlogic/pi-mono/issues/2458))
...
- Fixed project-local npm package updates to install npm `latest` instead of reusing stale saved dependency ranges, and added `Did you mean ...?` suggestions when `bailu update <source>` omits the configured npm or git source prefix ([#2459](https://github.com/badlogic/pi-mono/issues/2459))
...
- Extensions and SDK callers can reuse bailu's built-in local bash backend via `createLocalBashOperations()` for `user_bash` interception and custom bash integrations. See [docs/extensions.md#user_bash](docs/extensions.md#user_bash).
- Startup no longer updates unpinned npm and git packages automatically. Use `bailu update` explicitly, while interactive mode checks for updates in the background and notifies you when newer packages are available. See [README.md](README.md).

### Breaking Changes

- Changed package startup behavior so installed unpinned packages are no longer checked or updated during startup. Use `bailu update` to apply npm/git package updates, while interactive mode now checks for available package updates in the background and notifies you when updates are available ([#1963](https://github.com/badlogic/pi-mono/issues/1963))
...
- Added `createLocalBashOperations()` export so extensions and SDK callers can wrap bailu's built-in local bash backend for `user_bash` handling and other custom bash integrations ([#2299](https://github.com/badlogic/pi-mono/issues/2299))
...
- Faster startup by lazy-loading `@mariozechner/bailu-ai` provider SDKs on first use instead of import time ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Lazy-load built-in `@mariozechner/bailu-ai` provider modules and root provider wrappers so coding-agent startup no longer eagerly loads provider SDKs before first use ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Added `bailu uninstall` alias for `bailu install --uninstall` convenience
...
- Fixed example extensions to support `BAILU_CODING_AGENT_DIR` environment variable for custom agent directory paths ([#2009](https://github.com/badlogic/pi-mono/pull/2009) by [@smithbm2316](https://github.com/smithbm2316))
...
- Fixed explicit `bailu -e <path>` extensions losing command and tool conflicts to discovered extensions by giving CLI-loaded extensions higher precedence ([#1896](https://github.com/badlogic/pi-mono/issues/1896))
...
- Fixed `bailu config` misclassifying `~/.agents/skills` as project-scoped in non-git directories under `$HOME`, so toggling those skills no longer writes project overrides to `.bailu/settings.json` ([#1915](https://github.com/badlogic/pi-mono/issues/1915))
...
- Fixed parallel bailu processes failing with false "No API key found" errors due to immediate lockfile contention on `auth.json` and `settings.json` ([#1871](https://github.com/badlogic/pi-mono/issues/1871))
...
- Fixed `@mariozechner/bailu-ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level `@mariozechner/bailu-ai` entry. OAuth login and refresh must be imported from `@mariozechner/bailu-ai/oauth` ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading with the new `@mariozechner/bailu-ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed browser bundling compatibility for `@mariozechner/bailu-ai` by removing Node-only side effects from default browser import paths ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
## [0.55.4] - 2026-03-02
...
- Fixed SIGINT being delivered to bailu while the process is suspended (e.g. via `ctrl+z`), which could corrupt terminal state on resume ([#1668](https://github.com/badlogic/pi-mono/pull/1668) by [@aliou](https://github.com/aliou)).
...
- Added offline startup mode via `--offline` (or `BAILU_OFFLINE`) to disable startup network operations, with startup network timeouts to avoid hangs in restricted or offline environments.
...
- Resource precedence for extensions, skills, prompts, themes, and slash-command name collisions is now project-first (`cwd/.bailu`) before user-global (`~/.bailu/agent`). If you relied on global resources overriding project resources with the same names, rename or reorder your resources.
...
- Fixed `.bailu` folder being created unnecessarily when only reading settings. The folder is now only created when writing project-specific settings.
...
- Added default skill auto-discovery for `.agents/skills` locations. Bailu now discovers project skills from `.agents/skills` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo), and global skills from `~/.agents/skills`, in addition to existing `.bailu` skill paths.
...
- Fixed extension source parsing so dot-prefixed local paths (for example `.bailu/extensions/foo.ts`) are treated as local paths instead of git URLs
...
- `bailu <package> --help` and invalid subcommands now show helpful output instead of failing silently ([#1347](https://github.com/badlogic/pi-mono/pull/1347) by [@ferologics](https://github.com/ferologics))
...
- Local path support for `bailu install` and `bailu remove`, with relative path resolution against the settings file. See [docs/packages.md#local-paths](docs/packages.md#local-paths).
...
- Added local path support for `bailu install` and `bailu remove` with relative paths stored against the target settings file ([#1216](https://github.com/badlogic/pi-mono/issues/1216))
...
- **Clear on shrink setting**: New `terminal.clearOnShrink` setting keeps the editor and footer pinned to the bottom of the terminal when content shrinks. May cause some flicker due to redraws. Disabled by default. Enable via `/settings` or `BAILU_CLEAR_ON_SHRINK=1` env var.
...
- **Android/Termux support**: Bailu now runs on Android via Termux. Install with:
  ```bash
  pkg install nodejs termux-api git
  npm install -g @mariozechner/bailu-coding-agent
  mkdir -p ~/.bailu/agent
  echo "You are running on Android in Termux." > ~/.bailu/agent/AGENTS.md
  ```
...
- Added `BAILU_PACKAGE_DIR` environment variable to override package path for content-addressed package managers (Nix, Guix) where store paths tokenize poorly ([#1153](https://github.com/badlogic/pi-mono/pull/1153) by [@odysseus0](https://github.com/odysseus0))
...
- Fixed `bailu update` not updating npm/git packages when called without arguments ([#1151](https://github.com/badlogic/pi-mono/issues/1151))
...
- Fixed `bailu update <source>` installing packages locally when the source is only registered globally ([#1163](https://github.com/badlogic/pi-mono/pull/1163) by [@aliou](https://github.com/aliou))
...
- Added `BAILU_AI_ANTIGRAVITY_VERSION` environment variable documentation to help text ([#1129](https://github.com/badlogic/pi-mono/issues/1129))
...
- **OSC 52 clipboard support for SSH/mosh** - The `/copy` command now works over remote connections using the OSC 52 terminal escape sequence. No more clipboard frustration when using bailu over SSH. ([#1069](https://github.com/badlogic/pi-mono/issues/1069) by [@gturkoglu](https://github.com/gturkoglu))
...
- **Extended prompt caching**: `BAILU_CACHE_RETENTION=long` enables 1-hour caching for Anthropic (vs 5min default) and 24-hour for OpenAI (vs in-memory default). Only applies to direct API calls. See [README.md#prompt-caching](README.md#prompt-caching).
...
- Added `BAILU_CACHE_RETENTION` environment variable to control cache TTL for Anthropic (5m vs 1h) and OpenAI (in-memory vs 24h). Set to `long` for extended retention. ([#967](https://github.com/badlogic/pi-mono/issues/967))
...
- External edits to `settings.json` are now preserved when bailu reloads or saves unrelated settings. Previously, editing settings.json directly (e.g., removing a package from `packages` array) would be silently reverted on next bailu startup when automatic setters like `setLastChangelogVersion()` triggered a save.
...
- Bailu packages for bundling and installing extensions, skills, prompts, and themes. See [docs/packages.md](docs/packages.md).
...
- Removed `discoverAuthStorage` and `discoverModels` from the SDK. `AuthStorage` and `ModelRegistry` now default to `~/.bailu/agent` paths unless you pass an `agentDir` ([#645](https://github.com/badlogic/pi-mono/issues/645))
...
- Extension package management with `bailu install`, `bailu remove`, `bailu update`, and `bailu list` commands ([#645](https://github.com/badlogic/pi-mono/issues/645))
...
- Glob pattern support with minimatch in package filters, top-level settings arrays, and bailu manifest (e.g., `"!funky.json"`, `"*.ts"`) ([#645](https://github.com/badlogic/pi-mono/issues/645))
- `/reload` command to reload extensions, skills, prompts, and themes ([#645](https://github.com/badlogic/pi-mono/issues/645))
- `bailu config` command with TUI to enable/disable package and top-level resources via patterns ([#938](https://github.com/badlogic/pi-mono/issues/938))
...
- Added `BAILU_SHARE_VIEWER_URL` environment variable for custom share viewer URLs ([#889](https://github.com/badlogic/pi-mono/pull/889) by [@andresaraujo](https://github.com/andresaraujo))
...
- Fixed error messages showing hardcoded `~/.bailu/agent/` paths instead of respecting `BAILU_CODING_AGENT_DIR` ([#887](https://github.com/badlogic/pi-mono/pull/887) by [@aliou](https://github.com/aliou))
...
- Fixed bailu exiting with code 0 when cancelling resume session selection
...
- Updated the default system prompt wording to clarify the bailu harness and documentation scope.
...
- Removed `bailu-internal://` path resolution from the read tool.
...
- Fixed `BAILU_CODING_AGENT_DIR` env var not expanding tilde (`~`) to home directory ([#778](https://github.com/badlogic/pi-mono/pull/778) by [@aliou](https://github.com/aliou))
...
- Hardware cursor is now disabled by default for better terminal compatibility. Set `BAILU_HARDWARE_CURSOR=1` to enable (replaces `BAILU_NO_HARDWARE_CURSOR=1` which disabled it).
...
- `bailu-internal://` URL scheme in read tool for accessing internal documentation. The model can read files from the coding-agent package (README, docs, examples) to learn about extending bailu.
...
- Piped stdin now works correctly: `echo foo | bailu` is equivalent to `bailu -p foo`. When stdin is piped, print mode is automatically enabled since interactive mode requires a TTY ([#708](https://github.com/badlogic/pi-mono/issues/708))
...
- CLI process title is now set to `bailu` for easier process identification ([#742](https://github.com/badlogic/pi-mono/pull/742) by [@richardgill](https://github.com/richardgill))
...
- Extensions now load correctly in compiled Bun binary using `@mariozechner/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@mariozechner/bailu-tui`, `@mariozechner/bailu-ai`, `@mariozechner/bailu-coding-agent`) are accessible to extensions without filesystem node_modules.
...
- `BAILU_SKIP_VERSION_CHECK` environment variable to disable new version notifications at startup ([#549](https://github.com/badlogic/pi-mono/pull/549) by [@aos](https://github.com/aos))
...
- External edits to `settings.json` while bailu is running are now preserved when bailu saves settings ([#527](https://github.com/badlogic/pi-mono/pull/527) by [@ferologics](https://github.com/ferologics))
...
- Session picker (`bailu -r`) and `--session` flag now support searching/resuming by session ID (UUID prefix) ([#495](https://github.com/badlogic/pi-mono/issues/495) by [@arunsathiya](https://github.com/arunsathiya))
...
- Managed binaries (`fd`, `rg`) now stored in `~/.bailu/agent/bin/` instead of `tools/`, eliminating false deprecation warnings ([#470](https://github.com/badlogic/pi-mono/pull/470) by [@mcinteerj](https://github.com/mcinteerj))
...
- OAuth refresh no longer logs users out when multiple bailu instances are running ([#466](https://github.com/badlogic/pi-mono/pull/466) by [@Cursivez](https://github.com/Cursivez))
...
- `commands/` directories are automatically renamed to `prompts/` on startup (both `~/.bailu/agent/commands/` and `.bailu/commands/`)
...
```
# Before
~/.bailu/agent/hooks/*.ts       →  ~/.bailu/agent/extensions/*.ts
~/.bailu/agent/tools/*.ts       →  ~/.bailu/agent/extensions/*.ts
.bailu/hooks/*.ts               →  .bailu/extensions/*.ts
.bailu/tools/*.ts               →  .bailu/extensions/*.ts
```
...
3. **Subdirectory with package.json:** `extensions/myext/package.json` with `"bailu"` field → loads declared paths
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@mariozechner/bailu-coding-agent";
export default function (bailu: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@mariozechner/bailu-coding-agent";
const factory: CustomToolFactory = (bailu) => ({ name: "my_tool", ... });
export default factory;

// After (both are now extensions)
import type { ExtensionAPI } from "@mariozechner/bailu-coding-agent";
...
```bash
# Before
bailu --hook ./safety.ts --tool ./todo.ts

# After
bailu --extension ./safety.ts -e ./todo.ts
```
...
```
~/.bailu/agent/commands/*.md    →  ~/.bailu/agent/prompts/*.md
.bailu/commands/*.md            →  .bailu/prompts/*.md
```
...
- Configurable keybindings via `~/.bailu/agent/keybindings.json`. All keyboard shortcuts (editor navigation, deletion, app actions like model cycling, etc.) can now be customized. Supports multiple bindings per action. ([#405](https://github.com/badlogic/pi-mono/pull/405) by [@hjanuschka](https://github.com/hjanuschka))
...
- Terminal window title now displays "bailu - dirname" to identify which project session you're in ([#407](https://github.com/badlogic/pi-mono/pull/407) by [@kaofelix](https://github.com/kaofelix))
...
- `Attachment` type removed. Use `ImageContent` from `@mariozechner/bailu-ai` instead. Add images directly to message content arrays.
...
const authStorage = discoverAuthStorage(); // ~/.bailu/agent/auth.json
const modelRegistry = discoverModels(authStorage); // + ~/.bailu/agent/models.json
...
- **Sessions saved to wrong directory**: In v0.30.0, sessions were being saved to `~/.bailu/agent/` instead of `~/.bailu/agent/sessions/<encoded-cwd>/`, breaking `--resume` and `/resume`. Misplaced sessions are automatically migrated on startup. ([#320](https://github.com/badlogic/pi-mono/issues/320) by [@aliou](https://github.com/aliou))
...
- **SessionManager API**: The second parameter of `create()`, `continueRecent()`, and `list()` changed from `agentDir` to `sessionDir`. When provided, it specifies the session directory directly (no cwd encoding). When omitted, uses default (`~/.bailu/agent/sessions/<encoded-cwd>/`). `open()` no longer takes `agentDir`. ([#313](https://github.com/badlogic/pi-mono/pull/313))

### Added

- **`--session-dir` flag**: Use a custom directory for sessions instead of the default `~/.bailu/agent/sessions/<encoded-cwd>/`. Works with `-c` (continue) and `-r` (resume) flags. ([#313](https://github.com/badlogic/pi-mono/pull/313) by [@scutifer](https://github.com/scutifer))
...
- **Automatic custom system prompt loading**: Bailu now auto-loads `SYSTEM.md` files to replace the default system prompt. Project-local `.bailu/SYSTEM.md` takes precedence over global `~/.bailu/agent/SYSTEM.md`. CLI `--system-prompt` flag overrides both. ([#309](https://github.com/badlogic/pi-mono/issues/309))
...
- **Credential storage refactored**: API keys and OAuth tokens are now stored in `~/.bailu/agent/auth.json` instead of `oauth.json` and `settings.json`. Existing credentials are automatically migrated on first run. ([#296](https://github.com/badlogic/pi-mono/issues/296))
...
- **SDK API changes** ([#296](https://github.com/badlogic/pi-mono/issues/296)):
  ...
  - Use `getModel()` from `@mariozechner/bailu-ai` for built-in models, `modelRegistry.find()` for custom models + built-in models
...
- **Symlinked skill directories**: Skills in symlinked directories (e.g., `~/.bailu/agent/skills/my-skills -> /path/to/skills`) are now correctly discovered and loaded.

## [0.27.3] - 2025-12-24

### Added

- **API keys in settings.json**: Store API keys in `~/.bailu/agent/settings.json` under the `apiKeys` field (e.g., `{ "apiKeys": { "anthropic": "sk-..." } }`). Settings keys take priority over environment variables. ([#295](https://github.com/badlogic/pi-mono/issues/295))
...
- **Startup timing instrumentation**: Set `BAILU_TIMING=1` to see startup performance breakdown (interactive mode only).
...
- **Project-specific settings**: Settings now load from both `~/.bailu/agent/settings.json` (global) and `<cwd>/.bailu/settings.json` (project). Project settings override global with deep merge for nested objects. Project settings are read-only (for version control). ([#276](https://github.com/badlogic/pi-mono/pull/276))
...
- **Process suspension**: Press `Ctrl+Z` to suspend bailu and return to the shell. Resume with `fg` as usual. ([#267](https://github.com/badlogic/pi-mono/pull/267) by [@aliou](https://github.com/aliou))

- **Configurable skills directories**: Added granular control over skill sources with `enableCodexUser`, `enableClaudeUser`, `enableClaudeProject`, `enableBailuUser`, `enableBailuProject` toggles, plus `customDirectories` and `ignoredSkills` settings. ([#269](https://github.com/badlogic/pi-mono/pull/269) by [@nicobailon](https://github.com/nicobailon))
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@mariozechner/bailu-coding-agent` to reuse OAuth token storage and model resolution. ([#245](https://github.com/badlogic/pi-mono/issues/245))
...
- **Global AGENTS.md loaded twice**: Fixed global AGENTS.md being loaded twice when present in both `~/.bailu/agent/` and the current directory. ([#239](https://github.com/badlogic/pi-mono/pull/239) by [@aliou](https://github.com/aliou))
...
- **`getMarkdownTheme()` export**: Custom tools can now import `getMarkdownTheme()` from `@mariozechner/bailu-coding-agent` to use the same markdown styling as the main UI.
...
- **JSON mode stdout flush**: Fixed race condition where `bailu --mode json` could exit before all output was written to stdout, causing consumers to miss final events.
...
- **Custom tools now require `index.ts` entry point**: Auto-discovered custom tools must be in a subdirectory with an `index.ts` file. The old pattern `~/.bailu/agent/tools/mytool.ts` must become `~/.bailu/agent/tools/mytool/index.ts`. This allows multi-file tools to import helper modules. Explicit paths via `--tool` or `settings.json` still work with any `.ts` file.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@mariozechner/bailu-ai`, `@mariozechner/bailu-tui`, `@mariozechner/bailu-coding-agent`).
...
- Fixed custom tools failing to load from `~/.bailu/agent/tools/` when bailu is installed globally. Module imports (`@sinclair/typebox`, `@mariozechner/bailu-tui`, `@mariozechner/bailu-ai`) are now resolved via aliases.

## [0.23.0] - 2025-12-17

### Added

- **Custom tools**: Extend bailu with custom tools written in TypeScript. Tools can provide custom TUI rendering, interact with users via `pi.ui` (select, confirm, input, notify), and maintain state across sessions via `onSession` callback. See [docs/custom-tools.md](docs/custom-tools.md) and [examples/custom-tools/](examples/custom-tools/). ([#190](https://github.com/badlogic/pi-mono/issues/190))
...
- Updated `@mariozechner/bailu-ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. ([#200](https://github.com/badlogic/pi-mono/pull/200) by [@kim0](https://github.com/kim0))
...
- Updated `@mariozechner/bailu-ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@mariozechner/bailu-ai` with interleaved thinking support for Anthropic models.
...
- **Bailu skills now use `SKILL.md` convention**: Bailu skills must now be named `SKILL.md` inside a directory, matching Codex CLI format. Previously any `*.md` file was treated as a skill. Migrate by renaming `~/.bailu/agent/skills/foo.md` to `~/.bailu/agent/skills/foo/SKILL.md`.
...
- **Git branch indicator now works in subdirectories**: The footer's git branch detection now walks up the directory hierarchy to find the git root, so it works when running bailu from a subdirectory of a repository. ([#156](https://github.com/badlogic/pi-mono/issues/156))
...
- **Collapse changelog setting**: Add `"collapseChangelog": true` to `~/.bailu/agent/settings.json` to show a condensed "Updated to vX.Y.Z" message instead of the full changelog after updates. Use `/changelog` to view the full changelog. ([#148](https://github.com/badlogic/pi-mono/issues/148))
...
- **Flexible Windows shell configuration**: The bash tool now supports multiple shell sources beyond Git Bash. Resolution order: (1) custom `shellPath` in settings.json, (2) Git Bash in standard locations, (3) any bash.exe on PATH. This enables Cygwin, MSYS2, and other bash environments. Configure with `~/.bailu/agent/settings.json`: `{"shellPath": "C:\\cygwin64\\bin\\bash.exe"}`.
...
- **File-based Slash Commands**: Create custom reusable prompts as `.txt` files in `~/.bailu/slash-commands/`. Files become `/filename` commands with first-line descriptions. Supports `{{selection}}` placeholder for referencing selected/attached content.
...
- Added thinking level persistence. Default level stored in `~/.bailu/settings.json`, restored on startup. Per-session overrides saved in session files.
...
- Add custom model configuration via `~/.bailu/models.json`
