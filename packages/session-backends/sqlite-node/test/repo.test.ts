...
import * as storedValues from "@bailu/agent-core";
import * as sessionWrites from "@bailu/agent-core";
...
async function withTempDir<T>(run: (directory: string) => Promise<T>): Promise<T> {
	const directory = await mkdtemp(join(tmpdir(), "bailu-sqlite-session-"));
	try {
