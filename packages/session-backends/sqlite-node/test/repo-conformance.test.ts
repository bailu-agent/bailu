...
import { type ConformanceCase, createSessionRepoConformance } from "@bailu/agent-core/harness/session/testing";
...
async function createConformanceRepo() {
	currentDirectory = await mkdtemp(join(tmpdir(), "bailu-sqlite-session-repo-conformance-"));
...
async function createSharedContainerConformanceRepo() {
	currentSharedDirectory = await mkdtemp(join(tmpdir(), "bailu-sqlite-session-repo-shared-conformance-"));
	return new SqliteSessionRepo({
