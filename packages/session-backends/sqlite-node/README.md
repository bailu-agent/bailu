# @bailu/session-backend-sqlite-node

Node `node:sqlite` Session backend for `@bailu/agent-core`.

```ts
import { BACKGROUND_CONTEXT } from "@bailu/agent-core";
import {
  createNodeSqliteFactory,
  SqliteSessionRepo,
} from "@bailu/session-backend-sqlite-node";

const repository = new SqliteSessionRepo({
  directory: "/var/lib/bailu/sessions",
