...
Transport-neutral client for the experimental Bailu service protocol.

```ts
import { Client, type ByteTransportFactory } from "@bailu/client";
...
```ts
import { Client } from "@bailu/client";
...
```ts
import { discoverUnixServers } from "@bailu/client/unix";

const routes = await discoverUnixServers({ directory: "/run/user/1000/bailu" });
// [{ serverId: "...", path: "/run/user/1000/bailu/<serverId>.sock" }]
```
