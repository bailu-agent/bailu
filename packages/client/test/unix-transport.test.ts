...
import { parseServiceCall } from "@bailu/chord";
import { ClientMessageDecoder, encodeServerMessage, PROTOCOL_VERSION } from "@bailu/protocol";
...
async function makeSocketPath(): Promise<string> {
	const directory = await mkdtemp(join("/tmp", "bailu-client-transport-"));
