...
- Removed the unnecessary Chord dependency from bailu-ai by defining its exported `JsonValue` type directly.
...
- Changed the Anthropic, Azure OpenAI, Google Generative AI, Google Vertex, Mistral, OpenAI Chat Completions, and OpenAI Responses adapters to send Bailu's default `User-Agent` unless overridden ([#8305](https://github.com/earendil-works/pi/issues/8305)).
...
- Changed Kimi Coding requests to use bailu's runtime `User-Agent` header.
...
- Added explicit `TelemetryContext` propagation across stream, deferred, and image request options using the vendor-neutral `@bailu/telemetry` contract.
...
- Fixed the `bailu-ai` executable path to match npm registry metadata, avoiding repeated consumer lockfile changes ([#6812](https://github.com/earendil-works/pi/pull/6812) by [@jmfederico](https://github.com/jmfederico)).
...
- Added the dynamic Radius `bailu-messages` gateway provider with OAuth and credential-specific catalog refresh.
...
- Fixed OpenAI Codex user-agent construction to synchronously load Node OS metadata, avoiding a startup race that could report `bailu (browser)` in Node/Bun.
...
- The root entrypoint (`@bailu/ai`) is now core-only and side-effect free. The old global API moved to the temporary `@bailu/ai/compat` entrypoint, a strict superset of the root: switching a file's import path is the only migration step. Moved symbols include `stream`/`complete`/`streamSimple`/`completeSimple`, `getModel`/`getModels`/`getProviders` (now deprecated aliases of `getBuiltinModel`/`getBuiltinModels`/`getBuiltinProviders` from `@bailu/ai/providers/all`), `registerApiProvider`/`unregisterApiProviders`/`resetApiProviders`/`getApiProvider`, `getEnvApiKey`/`findEnvKeys`, `setBedrockProviderModule`, the per-API lazy stream wrappers (`anthropicMessagesApi`, ...), and the image-generation API.
...
- API implementation modules moved from `src/providers/` to `@bailu/ai/api/*`, renamed by API id (`anthropic` -> `api/anthropic-messages`, `google` -> `api/google-generative-ai`, `mistral` -> `api/mistral-conversations`, `amazon-bedrock` -> `api/bedrock-converse-stream`), each exporting exactly `stream` and `streamSimple`. The old per-impl export names (`streamAnthropic`, `streamSimpleAnthropic`, ...) and legacy raw API subpaths (`./anthropic`, `./google`, `./openai-completions`, ...) are gone; import raw API implementations through `@bailu/ai/api/*`.
- Removed the `@bailu/ai/base` selective-provider entrypoint; use the root/core APIs with explicit `createModels()` collections and provider factories for isolated bundles.
...
- To keep the old global API temporarily, change imports from `@bailu/ai` to `@bailu/ai/compat`. The compat entrypoint preserves `stream`/`complete`, generated catalog reads, API registry helpers, env API-key helpers, lazy API wrappers, and image globals, but it will be removed in a future release.
- To migrate to the new runtime, create a `Models` collection and call methods on it:

  ```ts
  import { builtinModels } from "@bailu/ai/providers/all";
...
- For an isolated provider set, register provider factories explicitly:
  ...
  ```ts
  import { createModels } from "@bailu/ai";
  import { anthropicProvider } from "@bailu/ai/providers/anthropic";

  const models = createModels();
  models.setProvider(anthropicProvider());
  ```

- To call a raw API implementation directly, import from `@bailu/ai/api/*` and pass a compatible model plus auth/options yourself. Raw API modules export `stream` and `streamSimple`; use `.result()` on the returned stream for `complete`/`completeSimple` behavior:

  ```ts
  import { streamSimple } from "@bailu/ai/api/anthropic-messages";
  import { getBuiltinModel } from "@bailu/ai/providers/all";
...
- One provider factory per built-in provider under `@bailu/ai/providers/*` (e.g. `anthropicProvider()`, `openrouterProvider()`), plus `@bailu/ai/providers/all` with `builtinProviders()`/`builtinModels()` and typed `getBuiltin*` catalog reads. Generated catalogs are split per provider, so importing one provider pulls one catalog; `sideEffects` metadata makes the package tree-shakeable.
...
- Added `@bailu/ai/base` and direct provider registration exports for bundlers that want selective provider transports without root built-in registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
- Added prompt caching for Mistral requests using the bailu session ID as `prompt_cache_key`, including cached-token usage and cost accounting ([#5854](https://github.com/earendil-works/pi/issues/5854)).
...
- Fixed provider retry controls so OpenAI Codex Responses honors `maxRetries`, SDK retries default to `0`, and quota/billing 429s are not retried behind Bailu's retry handling ([#4991](https://github.com/earendil-works/pi-mono/pull/4991) by [@mitsuhiko](https://github.com/mitsuhiko)).

## [0.75.5] - 2026-05-23

### Breaking Changes

- Changed `OAuthLoginCallbacks` to require `onDeviceCode` and `onSelect`, so OAuth providers can rely on bailu supplying device-code and selection UI callbacks ([#4788](https://github.com/earendil-works/pi-mono/pull/4788) by [@vegarsti](https://github.com/vegarsti)).
...
- Fixed thinking levels for Gemma 4 models to use `thinkingLevel` and map Bailu reasoning levels to the model's supported thinking levels ([#2903](https://github.com/badlogic/pi-mono/pull/2903) by [@aadishv](https://github.com/aadishv))
...
- Lazy-load built-in provider modules and root provider wrappers so importing `@mariozechner/bailu-ai` no longer eagerly loads provider SDKs, significantly reducing base startup cost without changing dependency installation footprint ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@mariozechner/bailu-ai/oauth` package exports to point directly at built `dist` files, avoiding broken TypeScript resolution through unpublished wrapper targets ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level package entry. Import OAuth login/refresh functions from `@mariozechner/bailu-ai/oauth` instead of `@mariozechner/bailu-ai` ([#1814](https://github.com/badlogic/pi-mono/issues/1814))
...
- Re-exported TypeBox `Type`, `Static`, and `TSchema` from `@mariozechner/bailu-ai` to match documentation and avoid duplicate TypeBox type identity issues in pnpm setups ([#1338](https://github.com/badlogic/pi-mono/issues/1338))
...
- Added `BAILU_AI_ANTIGRAVITY_VERSION` environment variable to override the Antigravity User-Agent version when Google updates their version requirements ([#1129](https://github.com/badlogic/pi-mono/issues/1129))
...
- Added `BAILU_CACHE_RETENTION` environment variable to control cache TTL for Anthropic (5m vs 1h) and OpenAI (in-memory vs 24h). Set to `long` for extended retention. Only applies to direct API calls (api.anthropic.com, api.openai.com). ([#967](https://github.com/badlogic/pi-mono/issues/967))
...
- Browser compatibility for bailu-ai: replaced top-level Node.js imports with dynamic imports for browser environments ([#873](https://github.com/badlogic/pi-mono/issues/873))
...
- OpenAI Codex: switched to bundled system prompt matching opencode, changed originator to "bailu", simplified prompt handling
...
- Exported OpenAI Codex utilities: `CacheMetadata`, `getCodexInstructions`, `getModelFamily`, `ModelFamily`, `buildCodexBailuBridge`, `buildCodexSystemPrompt`, `CodexSystemPrompt` ([#510](https://github.com/badlogic/pi-mono/pull/510) by [@mitsuhiko](https://github.com/mitsuhiko))
...
- **Agent API moved**: All agent functionality (`agentLoop`, `agentLoopContinue`, `AgentContext`, `AgentEvent`, `AgentTool`, `AgentToolResult`, etc.) has moved to `@mariozechner/bailu-agent-core`. Import from that package instead of `@mariozechner/bailu-ai`.
