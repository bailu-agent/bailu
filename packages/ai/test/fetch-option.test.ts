...
import { streamSimple as streamBailuMessages } from "../src/api/bailu-messages.ts";
...
describe("fetch stream option", () => {
	...
	it("uses fetch for Mistral, Codex SSE, and bailu-messages HTTP requests", async () => {
		const { custom, fallback } = mockFetches();
		...
		await streamBailuMessages(createModel("bailu-messages"), context, {
			apiKey: "test-key",
			fetch: custom,
		}).result();
