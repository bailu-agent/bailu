...
const BAILU_USER_AGENT = `bailu (${platform()} ${release()}; ${arch()})`;
...
describe("xAI Responses provider", () => {
	...
	it("uses /responses with bearer auth and xAI-compatible request fields", async () => {
		...
		expect(captured.headers.get("user-agent")).toBe(BAILU_USER_AGENT);
		expect(captured.headers.get("session_id")).toBe("bailu-session-123");
		expect(captured.body).toMatchObject({
			model: "grok-4.5",
			store: false,
			stream: true,
			prompt_cache_key: "bailu-session-123",
	...
	it("uses bailu's User-Agent by default for Responses requests", async () => {
		let userAgent: string | null = null;
		vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
			userAgent = new Request(input, init).headers.get("user-agent");
			return completedResponse();
		...
		expect(result.stopReason, result.errorMessage).toBe("stop");
		expect(userAgent).toBe(BAILU_USER_AGENT);
	});
	...
	it("uses bailu's User-Agent by default for Completions requests", async () => {
		expect(await captureCompletionsUserAgent()).toBe(BAILU_USER_AGENT);
	});
