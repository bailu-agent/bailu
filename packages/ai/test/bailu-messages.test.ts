...
import { type BailuMessagesOptions, stream, streamSimple } from "../src/api/bailu-messages.ts";
...
function createModel(baseUrl: string): Model<"bailu-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
		api: "bailu-messages",
...
describe("bailu-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		...
		} satisfies BailuMessagesOptions;
	...
	it("surfaces backend error responses with diagnostics", async () => {
		...
		expect(message.diagnostics?.[0]?.type).toBe("bailu_messages_response_failure");
...
describe("bailu-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("bailu-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "bailu-messages";
		expect(api).toBe("bailu-messages");
	});
});
