...
describe("Mistral HTTP transport", () => {
	it("serializes SDK-style payloads to the Mistral wire format", async () => {
		...
		expect(headers.get("user-agent")).toBe(BAILU_USER_AGENT);
			...
			messages: [
				{
					...
					content: [
						...
						{ type: "toolCall", id: "abc123456", name: "lookup", arguments: { query: "bailu" } },
					],
					usage: {
						input: 0,
						output: 0,
				...
				tool_calls: [
					{
						id: "abc123456",
						type: "function",
						function: { name: "lookup", arguments: '{"query":"bailu"}' },
						index: 0,
					},
				],
			},
