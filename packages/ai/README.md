...
```bash
npm install @bailu/ai
```

TypeBox exports are re-exported from `@bailu/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@bailu/ai';
...
import { openaiProvider } from '@bailu/ai/providers/openai';
import { openrouterProvider } from '@bailu/ai/providers/openrouter';
import { amazonBedrockProvider } from '@bailu/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@bailu/ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. bailu-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@bailu/ai';
...
API-key credentials use the same discriminator as bailu's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@bailu/ai';
...
```typescript
import { validateToolCall, type Tool } from '@bailu/ai';
...
```typescript
import { builtinImagesModels } from '@bailu/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@bailu/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@bailu/ai';
import { openAICompletionsApi } from '@bailu/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@bailu/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@bailu/ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are bailu thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@bailu/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@bailu/ai/api/<api-id>`.
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for bailu-controlled thinking values
  chatTemplateArgs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_args values for thinkingFormat: 'baseten'; use $var for bailu-controlled thinking values
...
```typescript
import { createModels, type Context } from '@bailu/ai';
import { anthropicProvider } from '@bailu/ai/providers/anthropic';
import { openaiProvider } from '@bailu/ai/providers/openai';
import { googleProvider } from '@bailu/ai/providers/google';
...
- `@bailu/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@bailu/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@bailu/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@bailu/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@bailu/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@bailu/ai/api/bedrock-converse-stream.lazy';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `BAILU_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @bailu/ai login              # interactive provider selection
npx @bailu/ai login anthropic    # login to specific provider
npx @bailu/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@bailu/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@bailu/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@bailu/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@bailu/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@bailu/ai`.
