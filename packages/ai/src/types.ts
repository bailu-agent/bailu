import type { TelemetryContext } from "@bailu/telemetry";
import type { AnthropicOptions } from "./api/anthropic-messages.ts";
import type { AzureOpenAIResponsesOptions } from "./api/azure-openai-responses.ts";
import type { BailuMessagesOptions } from "./api/bailu-messages.ts";
...
export interface StreamOptions extends ProviderRequestOptions<Model<Api>> {
	 ...
	 * SGLang, ...) receive parameters bailu does not model, e.g. `top_p`, `top_k`, `min_p`,
...
export interface ApiOptionsMap {
	...
	"bailu-messages": BailuMessagesOptions;
}
...
export interface OpenAICompletionsCompat {
	...
	/** Whether streamed responses include `finish_reason`. When false, bailu infers `stop` or `toolUse` when the stream ends. Default: true. */
	...
	thinkingFormat?:
		...
		| "string-thinking"
		| "ant-ling";
	/** Kwargs to send as `chat_template_kwargs` when `thinkingFormat` is `chat-template`. Use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for bailu-controlled thinking values. */
	chatTemplateKwargs?: Record<string, ChatTemplateKwargValue>;
	/** Arguments to send as `chat_template_args` when `thinkingFormat` is `baseten`. Use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for bailu-controlled thinking values. */
...
export interface Model<TApi extends Api> {
	...
	reasoning: boolean;
	/**
	 * Maps bailu thinking levels to provider/model-specific values.
