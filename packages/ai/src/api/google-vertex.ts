...
function buildHttpOptions(model: Model<"google-vertex">, optionsHeaders?: ProviderHeaders): HttpOptions | undefined {
	...
	const headers = providerHeadersToRecord({ "User-Agent": getBailuUserAgent(), ...model.headers, ...optionsHeaders });
	if (headers) {
...
function getDisabledThinkingConfig(model: Model<"google-vertex">): ThinkingConfig {
	...
	// thinkingLevel without includeThoughts so hidden thinking remains invisible to bailu.
	const geminiModel = model as unknown as Model<"google-generative-ai">;
