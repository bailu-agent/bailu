 ...
 * Defaults to "short" and uses BAILU_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("BAILU_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}
...
function createClient(
	...
	const compat = getCompat(model);
	const headers: ProviderHeaders = { "User-Agent": getBailuUserAgent(), ...model.headers };
