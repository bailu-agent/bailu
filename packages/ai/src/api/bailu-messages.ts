/**
 * bailu-messages API implementation.
 *
 * Streams bailu's own message protocol directly to a backend: the request is a
 ...
 * models.json custom provider with `"api": "bailu-messages"`.
 */
...
export interface BailuMessagesOptions extends StreamOptions {
...
type BailuMessagesUsage = AssistantMessage["usage"];
type BailuMessagesStopReason = AssistantMessage["stopReason"];

/** Impact summary of a server-side message rewrite (e.g. a gateway policy). */
export type BailuMessagesRewriteImpact = {
	policyId: string;
...
/** Serialized assistant-message event as sent by a bailu-messages backend. */
export type BailuMessagesEvent =
	| { type: "start" }
	...
	| {
			type: "done";
			reason: Extract<BailuMessagesStopReason, "stop" | "length" | "toolUse">;
			usage: BailuMessagesUsage;
			responseId?: string;
			providerThinkingLevel?: string;
			rewrite?: BailuMessagesRewriteImpact;
	  }
	| {
			type: "error";
			reason: Extract<BailuMessagesStopReason, "aborted" | "error">;
			usage: BailuMessagesUsage;
			errorMessage?: string;
			responseId?: string;
			providerThinkingLevel?: string;
...
export class BailuMessagesResponseError extends Error {
	code?: string;
	...
	constructor(message: string, code: string | undefined, diagnosticDetails: Record<string, unknown>) {
		super(message);
		this.name = "BailuMessagesResponseError";
		this.code = code;
		this.diagnosticDetails = diagnosticDetails;
	}
}

function parseBailuMessagesErrorBody(body: string): BailuMessagesErrorBody | undefined {
	try {
		const parsed = JSON.parse(body) as BailuMessagesErrorBody | null;
...
function formatBailuMessagesResponseError(
	response: Response,
	body: string,
	errorBody: BailuMessagesErrorBody | undefined,
): string {
...
function createBailuMessagesResponseError(
	model: Model<"bailu-messages">,
	url: URL,
	response: Response,
	body: string,
): BailuMessagesResponseError {
	const errorBody = parseBailuMessagesErrorBody(body);
	const code = typeof errorBody?.error?.code === "string" ? errorBody.error.code : undefined;
	return new BailuMessagesResponseError(formatBailuMessagesResponseError(response, body, errorBody), code, {
		version: 1,
...
function createEmptyUsage(): BailuMessagesUsage {
	return {
		input: 0,
		output: 0,
		cacheRead: 0,
...
function appendRewriteDiagnostic(message: AssistantMessage, rewrite: BailuMessagesRewriteImpact | undefined): void {
	if (!rewrite) {
		return;
	}
	appendAssistantMessageDiagnostic(message, {
		type: "bailu_messages_rewrite",
		timestamp: Date.now(),
		details: { ...rewrite },
	});
}

function createEventConverter(model: Model<"bailu-messages">) {
	...
	return (event: BailuMessagesEvent): AssistantMessageEvent => {
		switch (event.type) {
			case "done":
...
async function* readBailuMessagesEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<BailuMessagesEvent> {
	...
	try {
		while (true) {
			...
			while (split !== -1) {
				const event = parseBailuMessagesEvent(buffer.slice(0, split));
				if (event) {
					yield event;
				}
		...
		if (buffer.trim()) {
			const event = parseBailuMessagesEvent(buffer);
			if (event) {
				yield event;
			}
		}
...
function parseBailuMessagesEvent(raw: string): BailuMessagesEvent | undefined {
	const data = raw
		.split("\n")
	...
	return data && data !== "[DONE]" ? (JSON.parse(data) as BailuMessagesEvent) : undefined;
}

function createErrorEvent(model: Model<"bailu-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof BailuMessagesResponseError) {
		appendAssistantMessageDiagnostic(
			assistantMessage,
			createAssistantMessageDiagnostic("bailu_messages_response_failure", error, error.diagnosticDetails),
		);
	}
...
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention | undefined {
	...
	return getProviderEnvValue("BAILU_CACHE_RETENTION", env) === "long" ? "long" : undefined;
}

export const stream: StreamFunction<"bailu-messages", BailuMessagesOptions> = (
	model: Model<"bailu-messages">,
	context: Context,
	options?: BailuMessagesOptions,
	...
	void (async () => {
		try {
			...
			if (!response.ok) {
				...
				throw createBailuMessagesResponseError(model, url, response, body);
			}
			if (!response.body) {
				throw new Error(`${model.provider} response has no body`);
			}

			for await (const bailuEvent of readBailuMessagesEvents(response.body)) {
				const event = convertEvent(bailuEvent);
...
export const streamSimple: StreamFunction<"bailu-messages", SimpleStreamOptions> = (
	model: Model<"bailu-messages">,
	context: Context,
	options?: SimpleStreamOptions,
): AssistantMessageEventStream => {
	const extra = options as BailuMessagesOptions | undefined;
