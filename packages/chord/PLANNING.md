...
The current Bailu experiments prove many required behaviors, but Chord will be implemented from scratch. Existing source may be used as test and design evidence, not copied into this package. Compatibility with experimental APIs or wire messages is not a requirement.
...
@bailu/chord
        ↑
Bailu agent, protocol, server, coding agent, TUI, and future applications
```

Chord must:

- have no dependency on another Bailu workspace package;
- contain no imports from `@bailu/*` or relative paths outside `packages/chord`;
...
- be buildable, testable, packable, and usable without resolving another Bailu package.
...
Current experiments depend on Bailu's Harness `Context`, which Chord cannot import. Chord therefore needs a small neutral invocation context.
...
Applications may define their own context keys. A Bailu adapter can carry telemetry and authenticated identity through such keys without Chord knowing their types.
...
The wire protocol contains no server ID, session ID, attachment ID, route, user identity, or host kind. A Bailu router may wrap or forward Chord envelopes using its own control fields without parsing service business payloads.
...
- `@bailu/chord` must be externalized so a plugin uses the host's one runtime and branding symbols.
...
- Bundle output must not depend on Bailu's repository path aliases.
...
|---|---|
| `packages/coding-agent/src/experimental/services/connection.ts` | Bailu connection state, selected-session attachment, route rebinding, Bailu client adapter |
| `packages/coding-agent/src/experimental/services/server.ts` | server-wide session directory and management implementations |
| `packages/coding-agent/src/experimental/services/worker.ts` | Session worker host construction and Bailu protocol publication adapter |
...
| `source-resolver.ts` and Bailu internal process entrypoints | Bailu source execution and process policy |

`packages/agent/docs/plugins.md`, `packages/agent/docs/rpc.md`, the experimental service tests, and the remote plugin fixture are behavioral input. They are not normative Chord APIs. Once migration finishes, generic semantics should be documented in Chord and Bailu documents should cover only their host-specific contracts and adapters.

Migration should happen only after Chord passes its standalone conformance suite:

1. implement Chord without changing Bailu callers;
2. add thin Bailu adapters and migrate generic service imports;
3. migrate the experimental plugin host and loaders;
4. adapt Bailu's routed protocol to carry Chord envelopes;
...
If Node-only APIs are exported, they should use a separate package export such as `@bailu/chord/node` or `@bailu/chord/bundler`; importing the main runtime must not load Node-only modules.
...
- add package-boundary checks preventing Bailu imports; and
...
Tests bundle an application-neutral fixture with two opaque entries and a third-party dependency, load each independently, activate it, reload changed source, prove unchanged source hashes are stable, reject corrupt manifests/integrity, and prove output resolves no Bailu packages.
...
- Bailu adapters for context metadata and routed RPC transport;
...
Exit condition: Bailu depends on Chord, while Chord remains independently packable and contains no Bailu imports.
...
- Bailu host APIs or built-in Bailu services;
...
- backward compatibility with experimental Bailu APIs or wire formats.
...
None of these decisions should introduce Bailu concepts into Chord.

## 17. Definition of done

Chord's initial scope is complete when:

- a standalone application unrelated to Bailu can define local and remote singleton/keyed services;
...
- the packed package works outside the Bailu monorepo; and
- an automated boundary check proves that Chord has no imports or dependencies on the rest of Bailu.
