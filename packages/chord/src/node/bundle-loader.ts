...
function resolveExternalTarget(specifier: string, resolver?: FacetBundleExternalResolver): string {
	...
	if (specifier === "@bailu/chord") return new URL(`../index.${extension}`, import.meta.url).href;
	if (specifier === "@bailu/chord/context") {
		return new URL(`../context/index.${extension}`, import.meta.url).href;
	}
	if (specifier === "@bailu/chord/node") return new URL(`../node.${extension}`, import.meta.url).href;
