...
the Bailu monorepo, but it is not a Bailu package: it does not depend on any other Bailu
...
`@bailu/chord`. Import public types and general runtime APIs from the
package root. Context constants and functions live in
`@bailu/chord/context` because their generic names should not pollute
the root API.
...
Import the standalone delta primitive from `@bailu/chord/delta`:

```ts
import { apply, track } from "@bailu/chord/delta";
...
`@bailu/chord/bundler` uses esbuild to turn ESM or TypeScript application
...
```ts
import { bundleFacetPackage } from "@bailu/chord/bundler";
...
```ts
import { createFacetBundleLoader } from "@bailu/chord/node";
