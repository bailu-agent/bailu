...
| invocation | route |
| --- | --- |
| `bailu` (bare, in a directory) | ask server to create a session for cwd |
| `bailu --resume` | invoke the picker command at startup |
| `bailu --session <id>` | attach directly |
...
```text
fresh working copy
→ built-in providers      (@bailu/providers-builtin)
→ remote catalogue        (@bailu/providers-catalog)
→ models.json transform   (@bailu/providers-models-json)
→ auth/availability mark  (@bailu/auth)
→ validated state
```
...
```ts
import { object, array, string, int, oneOf, literal } from "@bailu/schema";
...
`@bailu/schema` re-exports TypeBox constructors as free functions so declarations stay
