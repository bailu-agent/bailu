...
bailu already does wire-is-delta one layer down. `BailuMessagesEvent` — the serialized form
a bailu-messages backend sends — has no `partial`:
...
`bailu-messages.ts` then rehydrates: it holds a local `partial`, mutates it per event
...
`message_update` from the bailu-ai event directly. They are out of scope here.
...
### 5.1 bailu-ai frames stay at the bailu-ai boundary; Chord ops cross replication boundaries

`AssistantMessageFrame` is bailu-ai's semantic delta vocabulary (`text_delta`, `text_end`, …). [Delta tracking §6](../01-delta/delta.md#6-there-is-no-frame-type) defines no second frame wrapper: in-process replication carries `Op[]`, and a wire adapter carries encoded `WireOp[]`. Bailu-ai frames stop at the fold; Chord ops cross the replication boundary.

`AssistantMessageFrame` is bailu-ai's own delta vocabulary and stays. What changes is
