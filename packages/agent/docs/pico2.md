...
Assumed knowledge: bailu-ai (`Models`, `streamSimple`, tool definitions, thinking levels, deferred
...
AssistantMessageFrame, DeferredHandle, Usage and Models reuse bailu-ai; AgentMessage/ThinkingLevel and
...
(scope, kind, namespace, key) tuples name the same address within the receiver's scope. `bailu` and
...
Bailu/application concerns, not Chord concepts.
...
**Telemetry** uses bailu's callback `TelemetryContext` (typed schemas, no second contract). Spans and
