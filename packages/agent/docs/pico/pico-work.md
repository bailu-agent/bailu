...
transcript order, stored model arrays concatenated, then bailu-ai tool results ordered by call index.
...
reject; new attempts clear scratch; persisted assistant frames use the bailu-ai compact frame encoder,
...
- `@bailu/chord` Context types and `@bailu/chord/context` helpers (1, 6, 8, 19)
- `@bailu/chord/delta` (15, 16: preview/watch only; not storage)
- `@bailu/ai`: `faux` provider for tests, `utils/estimate` for thresholds, `SystemMessage` (4, 9, 12)
