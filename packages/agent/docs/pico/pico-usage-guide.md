...
Durable agent harness for bailu: one session file, any number of conversations, every piece of work
...
```typescript
import type { Call } from '@bailu/agent';
import { BACKGROUND_CONTEXT, withCancel } from '@bailu/chord/context';
...
```bash
npm install @bailu/agent
```

## Quick Start

```typescript
import { Harness, JsonlStorage, type Call } from '@bailu/agent';
import { BACKGROUND_CONTEXT } from '@bailu/chord/context';
import { readTool, writeTool, bashTool } from '@bailu/agent/tools';
...
| option | what it is |
|---|---|
| `models` | a bailu-ai `Models` collection |
...
```typescript
import { generationKind } from '@bailu/agent/kinds';
...
bailu-ai models the system prompt as messages in the transcript: the first rendered prompt of a
...
with native mid-conversation system messages the entry goes out as one; elsewhere bailu-ai renders it
...
```typescript
import { Type, type Tool } from '@bailu/agent';
