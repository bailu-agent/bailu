...
`handoff` stores its message and starts a context; `system` stores the exact bailu-ai `SystemMessage`
...
assistant cut before all its results, request-local bailu-ai transformation supplies missing results;
...
```ts
import { AssistantMessageFrameEncoder, type AssistantMessageFrame } from "@bailu/ai";
...
bailu-ai may return `stopReason: "aborted"`. The kind handles its domain outcome normally; if a durable
...
import { createContextKey, withAbortSignal, withContextValue } from "@bailu/chord/context";
...
<harness>
[warn] output truncated: 2,000 of 51,204 lines shown; full output at /tmp/bailu/out-4421.log
</harness>
```
