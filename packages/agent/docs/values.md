...
await session.readList(workspaceEvents("bailu"), { limit: 100 }, context);
```
...
- namespace `bailu` and every `pi.*` namespace are reserved for built-ins by contract;
...
`AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` come from `@bailu/ai`. Do not define a second frame codec or reducer.
...
3. Namespace `bailu` and every `pi.*` are reserved by contract; every built-in namespace starts with `pi.`, and application use is a trusted-programming defect.
