...
**Application values/lists (outside the reserved `bailu`/`pi.*` namespaces)**: tree scope copies every current scalar value and every surviving list element; branch scope copies none. A `seq <= tipSeq` cutoff is forbidden as "historical reconstruction" — it is provably not one:
...
2. The closed classifier/driver of §1.2, exported from core and consumed by all three backends; unknown reserved namespaces (exact `bailu` or undeclared `pi.*`) fail the fork only when current surviving state exists, identically on every backend.
...
- current surviving state in an unknown reserved namespace (exact `bailu` or an undeclared `pi.*` scalar or surviving list element) fails the fork on every backend; the same namespace **set then deleted** before the fork fails no backend — including JSONL, where the dead history remains as physical lines and the disk fold must classify it as absent. Construct both cases via raw committed writes.
...
- one closed core classifier owns every namespace disposition; unknown reserved namespaces (exact `bailu` or undeclared `pi.*`) fail forks exactly when current surviving state exists, equivalently on all backends;
