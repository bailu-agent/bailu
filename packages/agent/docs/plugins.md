...
`@bailu/chord`. This document specifies how Bailu composes that runtime with Bailu-owned
...
→ @bailu/providers-builtin:session provides pi.models/singleton

env.use(Models)
→ @bailu/model-selection:tui requires pi.models/singleton

env.provideMany(QuestionDialogs)
→ @bailu/question:session provides pi.question-dialog/keyed

env.observe(QuestionDialogs, handler)
→ @bailu/question:tui requires pi.question-dialog/keyed
```
...
export function createAgentControllerRuntimeFacet(lane: AgentLane) {
	return defineFacet({
		id: "@bailu/agent-controller-runtime",
		setup(env) {
...
```text
fresh ProviderDraft
→ built-in provider contribution        (@bailu/providers-builtin)
→ remote catalogue contribution         (@bailu/providers-catalog)
→ models.json transformation            (@bailu/providers-models-json)
→ authentication/availability marking   (@bailu/auth)
