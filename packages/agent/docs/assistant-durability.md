...
- `AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` from `@bailu/ai`;
...
4. Reuse bailu-ai's canonical frame conversion and reduction semantics.
...
Create one bailu-ai encoder per provider stream and feed it every event in order:
...
Do not define a second harness frame codec or reducer. Persistence stores the exported bailu-ai value directly; hydration calls `reduceAssistantMessageFrames()`.
...
3. Every stored element is an exported bailu-ai `AssistantMessageFrame`.
