...
class FailingSpillExecutionEnv extends NodeExecutionEnv {
	override async createTempFile(
		...
		if (options?.prefix === "bailu-output-") {
...
describe("NodeExecutionEnv", () => {
	...
	it("expands home-relative paths and file URLs", async () => {
		const root = createTempDir();
		const env = new NodeExecutionEnv({ cwd: root });
		expect(getOrThrow(await env.absolutePath("~/bailu-node-env-test", BACKGROUND_CONTEXT))).toBe(
			join(homedir(), "bailu-node-env-test"),
		);
	...
	it.each([
		...
		["an empty override shadows the base value", { BAILU_SESSION_FILE: "" }, "x:"],
		[
			"a string override replaces the base value",
			{ BAILU_SESSION_FILE: "/sessions/current.jsonl" },
		...
		async (_description, overrides, expectedSessionFile) => {
			...
			const env = new NodeExecutionEnv({
				cwd: root,
				shellEnv: {
					BAILU_SESSION_FILE: "/stale/parent.jsonl",
					BAILU_CODING_AGENT: "true",
					BAILU_NODE_ENV_PRESERVED_TEST: "preserved",
				},
			});
			const collected = await collectShellOutput(
				env,
				`printf '%s:%s|%s|%s' "\${BAILU_SESSION_FILE+x}" "\${BAILU_SESSION_FILE-}" "$BAILU_CODING_AGENT" "$BAILU_NODE_ENV_PRESERVED_TEST"`,
				{ env: overrides },
				BACKGROUND_CONTEXT,
			);
	...
	it("can replace rather than inherit the default shell environment", async () => {
		const root = createTempDir();
		const inheritedKey = "BAILU_NODE_ENV_INHERITED_TEST";
		const configuredKey = "BAILU_NODE_ENV_CONFIGURED_TEST";
		const explicitKey = "BAILU_NODE_ENV_EXPLICIT_TEST";
