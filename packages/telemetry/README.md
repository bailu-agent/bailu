...
Vendor-neutral telemetry contracts and typed schema utilities for bailu packages.
...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Bailu packages pass telemetry contexts explicitly and define their domain schemas separately.
...
- [Bailu Package Integration](#bailu-package-integration)
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@bailu/telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@bailu/telemetry';
...
`@bailu/telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@bailu/telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@bailu/agent-core';
...
## Bailu Package Integration

Package ownership is intentionally split:

- `@bailu/telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@bailu/ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@bailu/agent-core` owns and exports the bailu AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
import {
  ...
  startAiSpan,
  startHarnessSpan,
} from '@bailu/agent-core';
```

The bailu schemas use bailu-owned `pi.ai.*`, `pi.harness.*`, and `pi.session.*` names. Adapters may translate them to backend conventions without changing the emitted bailu vocabulary.
