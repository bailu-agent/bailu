...
import { Type } from "@bailu/ai";
import { defineTool } from "@bailu/coding-agent";
...
const documentationPages = globSync("**/*.md", { cwd: docsRoot })
	...
	.sort()
	.map((path) => ({ path }));
const documentationAuditHarness = createBailuCodingAgentHarness({
...
describeEval("Coding agent documentation", { harness: documentationAuditHarness }, (it) => {
	it.for(documentationPages)("$path matches the implementation", { timeout: 300_000 }, async ({ path }, { run }) => {
		...
		const result = await run(`Audit one Bailu documentation page against the repository implementation.

Documentation page: ${documentationPath}
Repository root: ${repositoryRoot}

Read the complete documentation page. Identify its concrete claims about Bailu behavior, public APIs, configuration, commands, formats, defaults, and supported values.
