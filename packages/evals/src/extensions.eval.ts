...
import { type BailuCodingAgentInput, createBailuCodingAgentHarness } from "./bailu-harness.ts";
...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasBailuDocs: boolean;
	...
	return createBailuCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			return {
				...
				systemPromptHasBailuDocs: session.systemPrompt.includes("\nBailu documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Bailu system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Bailu system prompt has no working-directory section.");
	return defaultPrompt.slice(0, cwdStart);
}

const ExtensionAuthoringJudge = createJudge<BailuCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@bailu/coding-agent")) {
				failures.push("extension does not import the canonical @bailu/coding-agent package");
			}
...
const extensionHarnessTable = evalHarnessTable("Bailu extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"Bailu extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create a Bailu extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("Bailu eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasBailuDocs).toBe(expectsFullPrompt);
			});
		},
	);
});
