...
const artifactDirectory = process.env.BAILU_EVAL_ARTIFACT_DIR
	? resolve(packageRoot, process.env.BAILU_EVAL_ARTIFACT_DIR)
	: resolve(
			packageRoot,
			".eval",
...
if (hasCliModelSelection) {
	...
	provider = process.env.BAILU_PROVIDER?.trim() || undefined;
	model = process.env.BAILU_MODEL?.trim() || undefined;
	if (Boolean(provider) !== Boolean(model)) {
		console.error("Default model selection requires both BAILU_PROVIDER and BAILU_MODEL.");
		process.exit(1);
	}
}
...
const childEnvironment = {
	...process.env,
	BAILU_EVAL_ARTIFACT_DIR: artifactDirectory,
};
if (provider && model) {
	childEnvironment.BAILU_PROVIDER = provider;
	childEnvironment.BAILU_MODEL = model;
} else {
	delete childEnvironment.BAILU_PROVIDER;
	delete childEnvironment.BAILU_MODEL;
}
