...
Bailu evals are behavioral, model-backed checks for Bailu workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native Bailu session artifacts.
...
```bash
BAILU_PROVIDER=openai BAILU_MODEL=gpt-5.6-sol npm run eval
```
...
Authentication comes from Bailu's normal `ModelRuntime`, including Bailu subscription credentials and provider API-key
...
native Bailu session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. Bailu-specific evals use `createBailuCodingAgentHarness(...)` from `src/bailu-harness.ts`, with one harness bound
...
const harness = createBailuCodingAgentHarness({ noTools: "all" });

describeEval("Bailu smoke", { harness }, (it) => {
...
### Configuring the Bailu harness

`createBailuCodingAgentHarness(...)` accepts:
...
- `noTools`: Bailu's tool-disable configuration.
...
```ts
const harness = createBailuCodingAgentHarness({
...
prompt creates or changes Bailu resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create a Bailu extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic Bailu adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other Bailu configuration:
...
The Bailu harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook
