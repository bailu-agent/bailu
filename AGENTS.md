...
- Pre-commit blocks lockfile commits unless `BAILU_ALLOW_LOCKFILE_CHANGE=1`. Don't bypass unless the user wants the lockfile change committed.

## Git

Multiple bailu sessions may be running in this cwd at the same time, each modifying different files. Git operations that touch unstaged, staged, or untracked files outside your own changes will stomp on other sessions' work. Follow these rules:
...
For testing bailu's interactive mode, load and follow [.bailu/skills/interactive-testing.md](.bailu/skills/interactive-testing.md).
...
For release preparation, publishing, verification, or recovery, load and follow [.bailu/skills/release.md](.bailu/skills/release.md).
