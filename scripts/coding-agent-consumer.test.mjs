...
const codingAgentName = "@bailu/coding-agent";
const devPackages = ["client", "protocol", "server"].map((name) => `@bailu/${name}`);

function createFixture(t, { importServer = false, declareServer = false } = {}) {
	const root = mkdtempSync(join(tmpdir(), "bailu-consumer-test-"));
	t.after(() => rmSync(root, { recursive: true, force: true }));
	const packages = [codingAgentName, "@bailu/chord", ...devPackages].map((name) => ({
		name,
	...
	for (const pkg of packages) {
		...
		const manifest = {
			...
			...(isAgent ? {
				bin: { bailu: "dist/bundle/cli.js" },
				dependencies: {
					"@bailu/chord": "1.0.0",
					...(declareServer ? { "@bailu/server": "1.0.0" } : {}),
				},
...
${importServer ? 'import "@bailu/server";' : ""}
import { marker } from "@bailu/chord";
...
test("installs only coding-agent directly and uses overrides only for declared runtime dependencies", (t) => {
	...
	const nested = join(directory, "node_modules", codingAgentName, "node_modules/@bailu/server");
	mkdirSync(nested, { recursive: true });
	writeFileSync(join(nested, "package.json"), JSON.stringify({ name: "@bailu/server", version: "1.0.0" }));
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /bailu\/server must not be installed/);
...
test("fails when the SDK imports an undeclared server despite a working CLI", (t) => {
	...
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /Cannot find package '@bailu\/server'/);
});
