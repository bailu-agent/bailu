...
// Bump this only when generated model metadata requires behavior unavailable in older bailu clients.
...
function validateIndex(index) {
	...
	for (const catalog of index.catalogs) {
		if (
			...
			typeof catalog.minimumBailuVersion !== "string" ||
...
function compareBailuVersions(left, right) {
...
function buildIndex(existingIndex, publication) {
	const entry = {
		minimumBailuVersion: MINIMUM_PI_VERSION,
	...
	const catalogs = (existingIndex?.catalogs || [])
		.filter((catalog) => catalog.minimumBailuVersion !== MINIMUM_PI_VERSION)
		.concat(entry)
		.sort((left, right) => compareBailuVersions(left.minimumBailuVersion, right.minimumBailuVersion));
	return {
...
async function main() {
	...
	const temporaryDir = mkdtempSync(join(tmpdir(), "bailu-model-catalog-"));
	try {
		...
		const currentEntry = currentIndex?.catalogs.find(
			(catalog) => catalog.minimumBailuVersion === MINIMUM_PI_VERSION,
		);
