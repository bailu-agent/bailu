#!/usr/bin/env bash
#
# Build bailu binaries for all platforms locally.
...
#     bailu-darwin-arm64.tar.gz
#     bailu-darwin-x64.tar.gz
#     bailu-linux-x64.tar.gz
#     bailu-linux-arm64.tar.gz
#     bailu-windows-x64.zip
#     bailu-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before bailu starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/bailu.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/bailu"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating bailu-$platform.zip..."
        (cd "$platform" && zip -r ../bailu-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating bailu-$platform.tar.gz..."
        mv "$platform" bailu && tar -czf bailu-$platform.tar.gz bailu && mv bailu "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../bailu-$platform.zip)
    else
        tar -xzf bailu-$platform.tar.gz && mv bailu "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/bailu.exe"
    else
        echo "  $OUTPUT_DIR/$platform/bailu"
    fi
done
