...
async function check(t, manifest, source, extraFiles = {}) {
	const root = await mkdtemp(join(tmpdir(), "bailu-runtime-deps-"));
...
test("rejects undeclared imports even when the workspace package exists", async (t) => {
	const result = await check(t, {}, 'export { createUnixServer } from "@bailu/server/unix";', {
		"packages/server/package.json": JSON.stringify({ name: "@bailu/server", version: "1.0.0" }),
	});
	assert.equal(result.status, 1);
	assert.match(result.stderr, /src[\\/]index\.ts:1: @bailu\/server\/unix is not declared/);
});
