...
const agentDirEnvName = "BAILU_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "BAILU_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific BAILU_CODING_AGENT_DIR for the benchmark run
  ...
  --no-offline           Do not force BAILU_OFFLINE=1 / BAILU_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	const env = { ...process.env, BAILU_TIMING: "1" };
	if (options.agentDir) {
	...
	if (options.offline) {
		env.BAILU_OFFLINE = "1";
		env.BAILU_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "bailu-startup-benchmark-")) : undefined;
