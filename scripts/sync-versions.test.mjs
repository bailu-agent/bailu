...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	const root = await mkdtemp(join(tmpdir(), "bailu-sync-versions-"));
	try {
		...
		await writeManifest(root, "packages/evals", {
			...
			private: true,
			dependencies: {
				"@bailu/coding-agent": "^1.0.0",
				"@mariozechner/pi-ai": "npm:@bailu/ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@bailu/coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/pi-ai"], "npm:@bailu/ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@bailu/coding-agent"], "^1.0.0");
