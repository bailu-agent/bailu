...
const developmentPackages = new Set(["client", "protocol", "server"].map((name) => `@bailu/${name}`));
...
export function smokeTestCodingAgentConsumer(directory, runtime = process.execPath) {
	...
	const env = {
		...
		XDG_CACHE_HOME: home,
		BAILU_CODING_AGENT_DIR: join(home, ".bailu", "agent"),
		BAILU_OFFLINE: "1",
		BAILU_TELEMETRY: "0",
	};
...
for (const name of ["client", "protocol", "server"]) {
  assert.throws(() => import.meta.resolve("@bailu/" + name), /Cannot find|cannot find/, name + " must not be installed");
}
...
`);
		...
		for (const cli of new Set([manifest.bin.bailu, "dist/cli.js"])) {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	...
	const root = mkdtempSync(join(tmpdir(), "bailu-package-consumer-"));
	try {
