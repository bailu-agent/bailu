...
const packages = [
	{ directory: "packages/chord", name: "@bailu/chord" },
	{ directory: "packages/telemetry", name: "@bailu/telemetry" },
	{ directory: "packages/ai", name: "@bailu/ai" },
	{ directory: "packages/tui", name: "@bailu/tui" },
	{ directory: "packages/agent", name: "@bailu/agent-core" },
	{ directory: "packages/protocol", name: "@bailu/protocol" },
	{ directory: "packages/client", name: "@bailu/client" },
	{ directory: "packages/session-backends/sqlite-node", name: "@bailu/session-backend-sqlite-node" },
	{ directory: "packages/server", name: "@bailu/server" },
	{ directory: "packages/coding-agent", name: "@bailu/coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "bailu-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `bailu-${platform}.zip` : `bailu-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createBailuShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "bailu.cmd"))) {
			writeFileSync(join(installDirectory, "bailu.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\bailu.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "bailu.ps1"), '& "$PSScriptRoot/node_modules/.bin/bailu.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "bailu.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\bailu.exe" %*\r\n');
		writeFileSync(join(installDirectory, "bailu.ps1"), '& "$PSScriptRoot/node_modules/.bin/bailu.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "bailu"), join(installDirectory, "bailu"));
}
...
if (rootPackageJson.name !== "bailu-monorepo") {
...
if (!options.skipInstall) {
	...
	createBailuShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createBailuShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `bailu-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "bailu.exe" : "bailu")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "bailu.cmd" : "bailu")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "bailu.cmd" : "bailu")} --help`);
	}
}
